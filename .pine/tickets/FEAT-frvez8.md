...
Built a new `sdk/examples/reference-host` (server.mjs, tenant.html,
package.json, README) instead of extending host-page: the two-tenant
registry, per-tenant provisioning state, and webhook-fire/datastore
routes are a different app, and growing host-page would leave two
half-agreeing examples. host-page stays the handshake demo, untouched.
`docs/src/content/docs/guides/embedding.md` rewritten against the
reference code; docs build green with valid links. No sdk/src edits.

Proven once against packed SDK (npm pack + external install in /tmp, no
checkout) and a live sqlite server with auth + embed on: both tenants
provision (import -> credential -> attach headerAuth -> activate ->
datastore), embed-session mints kfe1 with exact origin/scopes/branding,
unknown slug 404s, fire -> succeeded -> datastore row read back filtered
per tenant, cross-tenant stream-ticket 404s with the host surviving,
birch reading acme's datastore 404s.

Deviations worth knowing: datastore writes are host-side REST (SDK has
no datastore methods; embed default-deny makes them backend-only), with
the landed kilasflow.datastore insert params documented for the swap.
Tenant 2 onboarding is operator store-layer/SQL (no signup or invite
endpoint exists). Found while proving: same-label active workflows
refuse activation per tenant (reference host retires namesakes; guide
says so), and execution terminal statuses are
succeeded/failed/cancelled. Flagged to DatastoreAPI (routes
registration) and resolved.
...
```
 ...
 .pine/tickets/FEAT-1axhdn.md                       |   42 +-
 ...
 .pine/tickets/FEAT-cjpbe6.md                       |   41 +-
 ...
 .pine/tickets/FEAT-frvez8.md                       |   98 +
 ...
 .pine/tickets/FEAT-k9dwgn.md                       |   33 +-
 ...
 internal/api/routes.go                             |   55 +-
 internal/api/server.go                             |   49 +-
 ...
 internal/engine/runner.go                          |  253 +-
 internal/engine/service.go                         |  341 +-
 ...
 730 files changed, 120486 insertions(+), 2199 deletions(-)
```
