...
## Notes (AuthorGuide, 2026-09-06)

- Ownership: docs packs pages only — `guides/node-authoring.md` (tutorial) + `reference/node-packs.md` (format reference). No code edits.
- Vocabulary kept to toolchain terms: scaffold/validate/pack subcommands on `cmd/nodepackgen`, checksum approval via `pack.sha256` sidecar, reserved `kilasflow.` prefix + `resource`/`operation` cascade keys.
- Flow is author -> validate -> install per parent correction. Transcripts are real runs against a built `nodepackgen`: scaffold output, `packs are valid`, `wrote pack.sha256`, and the `sha256sum`-shape sidecar bytes.
- Every guide JSON block extracted and re-validated exactly as printed: stages 1-3 clean; trigger block reports only the throwaway-registry harness gap (`bound to executor "core.packTrigger", which this server has not installed`) — the shipped WAHA trigger produces the identical line, and the guide says so. Trigger HMAC corrected to `sha512` (only implemented algorithm) and option entries to `value`/`label` during verification.
- Manifest field table taken from the validator's known-key lists (`packKeys`, `resourceKeys`, `operationKeys`, `parameterKeys`, `provenanceKeys`, `triggerKeys`); troubleshooting diagnostics pasted verbatim from CLI runs. Licence note follows the sidecar position (operator-installed, never distributed).
- `pnpm build` green, starlight links validator: all internal links valid.
...
```
 ...
 725 files changed, 118238 insertions(+), 2152 deletions(-)
```
