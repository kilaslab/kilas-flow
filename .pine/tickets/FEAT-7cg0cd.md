...
- Reference checkout (read-only, never vendored): `$KILASFLOW_N8N_REFERENCE/pnpm-lock.yaml` (`isolated-vm@6.1.2` under `packages/@n8n/expression-runtime`, `packages/cli`, `packages/nodes-base`), `$KILASFLOW_N8N_REFERENCE/packages/workflow/package.json` (`LicenseRef-n8n-sustainable-use`, `@n8n/expression-runtime` dependency).
- Owner's community package (read-only): `<owner-community-package>/package.json` — the `n8n` manifest key and the `n8n-workflow` peer dependency this loader must satisfy.
