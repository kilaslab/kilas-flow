...
- **Nothing user-authored is committed.** The baseline names a body `<template id>/<node position>` and records outcomes, reason classes and the analyser's own refusal subjects; a test proves an author's identifier never reaches `baseline.json` or `BASELINE.md`.
...
- `scripts/js-diff/harness.mjs`, `Makefile` (`js-corpus`, `js-corpus-baseline`, `js-corpus-check`, `js-diff`), `.github/workflows/ci.yml`
