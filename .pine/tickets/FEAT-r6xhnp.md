...
Configuration: add `table_prefix` to `config.Database` and thread it to both `database.Open` (for the namer) and the migration runner (for the SQL). Validate it — letters, digits and underscore only, ending in `_` — and **cap its length** before it is ever interpolated into DDL. The cap is not cosmetic: V2-p9-1 creates one physical table per datastore at run time, so the identifier budget must hold for names this ticket never sees. Compute the worst case at the maximum configured prefix across every generated table, index and constraint name, and assert it, because PostgreSQL truncates silently rather than erroring — two names that differ only past byte 63 collapse into one, and `CREATE INDEX IF NOT EXISTS` then skips the second without complaint while the code believes the index exists.
...
Status: doing. Implemented within Persistence ownership; needs integrator
wiring nowhere — Migrate/Open signatures unchanged, prefix rides the handle.

Done:
- `database.table_prefix` in `internal/config/config.go` (Database section):
  empty default, `koanf:"table_prefix"`, validated (lowercase/digits/_
  ending `_`, cap `MaxTablePrefixLength = 16` for the 63-byte PG limit;
  longest migration identifier is 40 chars,
  16 + 40 = 56, leaving room for runtime datastore tables).
- `database.Open` sets `NamingStrategy{TablePrefix}`; all 13 models (ticket
  says seven — the tree now has thirteen) converted `TableName() string` to
  `TableName(namer schema.Namer)` returning `namer.TableName("<base>")`.
- Migration runner substitutes the prefix at run time: defined identifiers
  (CREATE TABLE / CREATE INDEX / CONSTRAINT names) are collected from the
  migration files themselves and every quoted occurrence rewritten.
  Empty prefix returns statements untouched (byte-identical default).
  `schema_migrations` stays unprefixed (per-database bookkeeping).
- Startup refusal (`checkTablePrefix`): proceeds when prefixed tables exist;
  refuses when the full bare baseline exists under a non-empty prefix, or
  when version history exists without matching tables (covers prefix A vs B).
  Deliberately NOT any-single-table: a host app owning workflows/executions/
  credentials is coexistence, and the criterion demands it succeed.
- Struct tags untouched: migrations are authoritative, tags are documentation
  + drift-test input. Editing them changes nothing at runtime (the trap the
  plan names).
- Ran `go run ./scripts/config-reference.go` per OperatorGuide (FEAT-27km39):
  `config.example.yaml` regenerated with `table_prefix: ''`.

Tests (all passing):
- `internal/repository/table_names_test.go`: round-trip identity per model,
  empty + `kflow_` (incl. workflow_versions, webhook_bindings,
  execution_node_runs).
- `internal/repository/prefix_test.go`: SaveDraft/Get/List through the store
  against `kflow_` tables; bare names absent.
- `internal/database/prefix_test.go`: empty-prefix byte identity (both
  dialects, up+down); columns/literals untouched; 13 literals covered;
  63-byte budget at max prefix; coexistence (`kflow_` + host tables +
  host `idx_workflows_tenant_updated` intact + full enumeration of tables,
  indexes, FK targets); mismatch refusal both directions; prefixed rollback.
- Full existing suites green: `go test -count=1 ./internal/database/...
  ./internal/repository/... ./internal/sqlnode/...` all ok. Live-Postgres
  variants skip (no `KILASFLOW_TEST_POSTGRES_DSN` here); PG substitution is
  the same code path with `"` quotes.

Notes for integrator:
- Prefix is reachable at runtime without GORM via `config.Database.TablePrefix`
  (p6-4 LISTEN/NOTIFY + advisory-lock keying).
- A prefix remains a naming convention, not isolation (p6-5 owns the guard).
...
- Commits (4):
  ...
  - `29074c69` — fix(engine): never run the branch that was not taken
...
```
 ...
 735 files changed, 139013 insertions(+), 2635 deletions(-)
```
