...
Closed by `pine close --evidence` on 2026-09-23. Rewritten on 2026-09-23 to list only this ticket's own commits (`git log --grep "BUG-2eryxn" 2f8e9b1..5830b4e`) and the files exactly those commits changed.

- Range: `2f8e9b1..5830b4e`
- Commits (2):
  - `5b77b6e2` — chore(pine): close BUG-2eryxn with its landing evidence
  - `8c388d03` — BUG-2eryxn: an MCP tool's boolean argument reaches its flag, both true and false
- Files changed by those commits (merge commits excluded):

```
 .pine/tickets/BUG-2eryxn.md | 309 +++++++++++++++++++++++++++++++++++++++++++-
 internal/cli/mcp.go         |   5 +-
 internal/cli/mcp_test.go    | 157 +++++++++++++++++-----
 3 files changed, 429 insertions(+), 42 deletions(-)
```
