...
- `config.Datastore` block landed (`internal/config/config.go`): single-word
  section per the envKeyToPath rule, four bounds
  (`max_datastores_per_tenant`, `max_columns_per_datastore`,
  `max_rows_per_datastore`, `max_value_bytes`), defaults 100 / 100 /
  100000 / 1MiB in `Default()`, `Validate` rejects zero or negative with
  the key named. The numbers repeat `datastore.DefaultLimits` rather than
  importing them (that import would cycle: datastore imports config for
  `MaxTablePrefixLength`); `TestConfigDatastoreDefaultsMatchDefaultLimits`
  in the datastore package pins the two together.
- Table-driven config tests: `TestDatastoreDefaultsBoundGrowth`,
  `TestValidateRejectsBadDatastoreBounds` (8 zero/negative cases),
  `TestDatastoreBoundsAreReachableFromYAMLAndTheEnvironment` (file +
  `KILASFLOW_DATASTORE_*` env). `scripts/config-reference.go` regenerated
  twice under the last-regen-wins protocol with SurfaceFinish (check clean,
  both datastore + secrets sections present). Engine wiring in
  `cmd/kilasflow/main.go`: `SetLimits` from `cfg.Datastore` beside
  `NewEngine` (Validate already refused non-positive, so it cannot fail
  on an accepted config).
- Hold-time criterion: new `TestRowLimitUsesABoundedProbe` pins the
  construction deterministically via the emit hook — the refused insert
  emits `SELECT 1 .. LIMIT 1 OFFSET ?` and no `COUNT(*)` — on both
  dialects. No wall-clock concurrent-reader test: the ticket states no
  numeric bound ("beyond the stated bound" names none), so a timing
  assertion would invent the bound it claims to check and be flaky by
  nature; the probe stops at the limit-th row whatever the table holds
  past it. Recorded here as the honest reading.
- `TestUsageObservable` (prior slice) covers PG-exact bytes
  (`pg_total_relation_size`, positive figure) and SQLite unavailability
  (`SizeKnown:false`, never zero); re-verified green on postgres in this
  slice via the pgvector/pg17 run.
- By hand: live 422 refusal naming limit+count (`value for column
  "api_key" is 42 bytes, past the maximum 32` with
  `KILASFLOW_DATASTORE_MAX_VALUE_BYTES=32`); boot refusal
  (`datastore.max_rows_per_datastore 0 must be positive`, exit 1);
  `make smoke-sqlite` → passed. `make smoke-postgres` red for the
  unrelated pgvector/compose reason recorded on FEAT-cjpbe6.
- No auto-expiry: breach refuses, nothing is evicted (unchanged).
...
```
 ...
 902 files changed, 176126 insertions(+), 3012 deletions(-)
```
