...
# Acceptance Criteria
- [x] Pasted n8n JSON goes through the server importer (or a shared translator) and yields the same nodes and parameters as Import
- [x] A Manual Trigger pastes as a manual trigger; a JS Code node becomes foreignCode (or jsCode once the runtime exists), never a Go Code node with the source lost
- [x] `=`-prefixed values become expressions
- [x] Paste shows the same import diagnostics
...
- `n8n.ImportFragment` (internal/interop/n8n/n8n.go) is `Import` with the document-level report entries a paste has no use for left out (instance `meta`, `settings`, `staticData`); pinned data stays reported. One translator, so a paste cannot drift from an import.
- `POST /api/v1/workflows/convert` (`convert-workflow-fragment`, internal/api/handlers/interop.go) answers `{nodes, connections, unsupported}` against the tenant's catalogue and saves nothing. Scope arm in internal/api/middleware/scope.go: `workflow:read`, matched before the `/workflows/{id}` prefix arm, like `/workflows/validate`. Documented in the contract page, the generated reference (whose embed verdicts for convert, validate and duplicate were corrected) and the n8n migration guide.
- Web: `clipboard.ts` keeps no n8n translator. `readClipboard` only classifies (own fragment or KilasFlow document vs n8n JSON); `convertN8n` calls the endpoint; `pasteInto` rewrites the report to the ids and names the nodes landed with (by id, or by payload name for connection entries that carry none). The editor's paste is async with a sequence ticket, so a slower answer never overwrites a newer paste and a paste that lands while locked is dropped; the status line summarises the report and "View paste report" opens it in the import report's own component (moved to `$lib/components/workflow-editor/` with `diagnostics-section.svelte`). A refused paste names the importer's reason and changes nothing.
- Tests: `fragment_import_test.go` (parity with Import, the three client-side failures, metadata vs pinData, refusals), `internal/api/convert_fragment_test.go`, scope cases, `clipboard.test.ts`, e2e `n8n-paste.spec.ts` (the ticket's reproduction, saved document asserted; refused paste).
- Reviewed in two rounds; round 1 found the rename gap for connection entries and the wrong embed verdict, both fixed in 61d446d; round 2 clean.
...
- Commits (4):
  - `61d446d3` — BUG-txafja: the paste report names a connection's node as it landed, and the reference states convert's embed scope
  - `5a435f78` — BUG-txafja: n8n nodes pasted onto the canvas go through the importer and arrive with its report
  - `47f3ddb7` — chore(pine): BUG-ngt25j closes with its evidence
...
```
 ...
 .pine/tickets/BUG-ngt25j.md                        |  732 ++++
 ...
 docs/src/content/docs/guides/n8n-migration.md      |  192 +-
 ...
 docs/src/content/docs/reference/api-contract.md    |   22 +-
 ...
 web/messages/en/editor.json                        |   45 +-
 web/messages/en/executions.json                    |    3 +
 web/messages/id/editor.json                        |   55 +-
 ...
 .../workflow-editor/workflow-editor.svelte         |  130 +-
 ...
 683 files changed, 92559 insertions(+), 2006 deletions(-)
```
