...
- [x] `pnpm test` is reachable from a Makefile target so the frontend suite is run by hand alongside `make lint` rather than by memory.
...
Last criterion verified and closed: `web-test` exists at `Makefile:140-142`
(`cd $(WEB_DIR) && pnpm test`) and `lint` runs `cd $(WEB_DIR) && pnpm test`
right after `pnpm check` (`Makefile:248-249`), so the frontend suite runs with
lint rather than by memory. Ran `make web-test` by hand: 25 files, 311 tests,
all passing. No Makefile edit was needed — the target the worktree note
proposed is already in the main tree, including the `lint` line.
...
- Commits (4):
  - `8f996133` — build: run the frontend suite from make
  - `3a211cad` — merge: give the dashboard lists one shell and one table (FEAT-ptyh9w)
  - `8e7c3db6` — refactor(web): give the dashboard lists one shell and one table
...
```
 ...
 708 files changed, 111193 insertions(+), 2709 deletions(-)
```
