...
# Acceptance Criteria
- [x] With `credentialId`, the stored `AllowedDomains` apply, or their intersection with the body's.
- [x] Placeholders are not merged when a host-like field (host, baseUrl, url) differs from the stored value.
- [x] `credentialId` is validated before the slot is claimed, and there is a per-tenant cap on concurrent tests.
- [x] Tests cover each case.

## Fix (2026-09-25)

Merged in 8ce5ea3 (branch commit aab0fcb; the claim merged with BUG-xpr3jj's token-owned idempotent release). `credentialId` is validated tenant-scoped and type-matched before the slot is claimed (unknown → 404, nothing dials); a placeholder is refused when host, port, baseUrl or url differ from the stored value; a test that merged a stored secret runs under the intersection of stored and body scope (`credentials.IntersectDomains`), refused when empty; a tenant runs at most 4 tests at once across both routes (429); the web form sends its scope with a test. Tests: internal/api/credentials_probe_scope_test.go. Review: clean.
...
- Commits (3):
  ...
  - `aab0fcb3` — BUG-pder07: testing an unsaved edit keeps a stored secret on its stored target and scope, and a tenant runs at most four tests at once
...
```
 ...
 383 files changed, 49820 insertions(+), 3856 deletions(-)
```
