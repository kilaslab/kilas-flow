...
# Acceptance Criteria
- [x] The configured auth header is redacted by name before the payload is stored.
- [x] A test covers it.

## Fix (2026-09-25)

Merged in 28e1da6 (branch commit 460a7e7). After a successful constant-time comparison, `authenticate` records the verified header's name; `readDelivery` stores its value as `[redacted]`; other headers are kept. Test: internal/webhook/header_auth_redaction_test.go (secret absent from the stored record, wrong secret still 401). Review: clean.
...
- Commits (3):
  ...
  - `460a7e7b` — BUG-cmnsfz: a header-auth trigger's verified header is stored redacted, whatever its name
...
```
 ...
 417 files changed, 54217 insertions(+), 3988 deletions(-)
```
