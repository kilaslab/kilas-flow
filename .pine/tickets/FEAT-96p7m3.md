...
Reference grounded in n8n 2.34.0 (read-only): ChainLlm (`promptType` auto default / define, `text`, `messages.messageValues` fixedCollection rows type ai|system|human + message, `hasOutputParser` toggle gating `ai_outputParser`, inputs main + ai_languageModel required/max1, batching.batchSize/delayBetweenBatches, one `processItem` call per item). `fixedCollection` already exists in `internal/node/registry.go` (`PropertyFixedCollection` + `PropertyGroup`), so the ticket's p2-2 dependency is satisfied with no new kind. No code copied.

Done (all in owned files):
- `kilasflow.chainLlm` registered (`chainLlmNode()` in nodes/ai.go, one-line `RegisterAll` entry in nodes/core.go flagged below, `core.chainLlm` executor entry in nodes/executors.go): main in/out, `model` ai_languageModel required/max1, `outputParser` ai_outputParser optional/max1, NO memory slot (deliberate parity fact, commented).
- Both prompt modes: `promptType` auto (text from named `inputField`, default `chatInput`) / define (`text` + ordered `messages.messageValues` rows System/Human/AI with expression-supporting text via the generic form — `expression.Resolve` recurses, no chain-specific UI).
- `ChainExecutor` drives `ai.ChatModel.Complete`/`Stream` directly (never `LoopRuntime`): exactly one model call per item, `ai.model.started/completed` (+`delta`) events per item on the live feed, `{"output": text}` per item.
- Model resolution reuses the factored `modelBackend.resolveModel` (descriptor from the model port by name via `soleDescriptor(input["model"])`, credential id re-resolved, never the key). Trap covered by `TestChainRequiresAModelOnTheModelPort`.
- Output parser slot: anything connected is refused naming the port (no parser node type exists, so nothing can connect; silent unstructured text would be the worse failure). Batching params (`batching.*`) not implemented — same importer-unmapped treatment as the agent.
- Validation names the row: `message N has empty text` / `message N has type ...`; define-with-nothing is refused; auto mode needs no rows.
- Editor icon: `builtin:link` + `link: Link` GLYPHS entry in node-visual.ts (chain accent `#0ea5e9`, own tile, no fallback grey).
- New tests: one-call-per-item via recorded provider bodies + event counts, typed rows + row expression resolution, validation row naming, model-port trap, empty-prompt refusal (zero provider calls), parser refusal.
- `go test -count=1 ./internal/ai/... ./nodes/...` green; `go build ./...` green.

Ownership notes:
- `nodes/core.go` one-line `chainLlmNode()` registration and `nodes/executors.go` chain entry are within the ticket's contract (chain entry explicitly owned); no other files outside ownership were edited.
- For integrator/API-Contract: `kilasflow.chainLlm` is a new node type — generated API models / OpenAPI spec assertions may need regen (not touched: API-Contract ownership).
- For importer: map n8n `chainLlm` promptType/text/messages.messageValues onto these keys (names chosen to match n8n verbatim); report `batching.*` and `hasOutputParser`/`needsFallback` as unmapped.
- Decision taken: both prompt modes implemented (field + message list), as the ticket recommended.

## Progress — AIClusterParity integrator follow-up 2026-09-06 (per Main)

- Importer: new `@n8n/n8n-nodes-langchain.chainLlm ↔ kilasflow.chainLlm` mapping (`exportTypeVersion` 1.9, the latest the 2.34.0 checkout publishes). `chainToKilas` carries `promptType`/`text`/`messages.messageValues` verbatim (keys chosen to match n8n, so this is nearly identity) with deep per-row expression conversion both ways (`chainMessagesToKilas`/`chainMessagesToN8N`); image rows are reported `dropped` and skipped rather than carried as empty rows the chain would refuse to compile. `hasOutputParser`/`needsFallback` arrive as `blocking`, top-level `batching` as `dropped`. `chainToN8N` writes the surface back and reports a renamed `inputField` as `lossy` (n8n auto always reads `chatInput`). Covered by `TestImportCarriesTheChainNode` (params, row expression translation, all three diagnostics).
- Advertisement list + `n8n_cluster_nodes.json` transcription updated (chain facts verified against the checkout today: versions 1–1.9, promptType auto|define, row keys type|message with ai|system|human values, batching keys).
- Regen: not needed — no API schema types changed; `generate-api`/`generate-types` output unaffected. `docs/`, `web/`, `sdk/`, `scripts/` untouched.
- Verify: `go build ./...` green; interop suite green (incl. round-trip stability through the new mapping); `gofmt -l` clean. Forbidden paths untouched.
...
```
 ...
 735 files changed, 138264 insertions(+), 2635 deletions(-)
```
