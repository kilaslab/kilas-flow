...
- n8n 2.34.0 reference checkout (read-only, outside this repo): `$KILASFLOW_N8N_REFERENCE/packages/nodes-base/nodes/Schedule/ScheduleTrigger.node.ts` (`version: [1, 1.1, 1.2, 1.3]`, the emitted item's field names) and `.../Schedule/GenericFunctions.ts` (`withIntervalDefaults` defaults, `validateInterval` ranges).
