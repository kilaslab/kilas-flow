...
## Progress (GuardTier, 2026-09-06)

Status: doing. Implemented within GuardTier ownership (`internal/sqlnode`
only); startup wiring below is for the integrator (main owner).

Done in `internal/sqlnode/sqlnode.go`:
- `Guard` gains `Internal *InternalTarget` (driver + host + port + database
  name), alongside the existing `InternalPaths`. Documents that a table
  prefix does not scope the refusal (naming, not a boundary).
- `ParseInternalTarget(driver, dsn)` normalises the installation's own
  postgres URL once at startup (default port 5432, both postgres/postgresql
  schemes, IPv6, options ignored — sslmode changes wrapping, not the
  target). Anything unparsable, or a non-postgres driver, is an error the
  caller must treat as fatal (refuse boot), never as an empty guard.
- `checkInternalDatabase` compares resolved identity, not DSN text: exact
  database name, normalised port, then same-spelling backstop (covers DNS
  round-robin handing two answers for one name) plus resolved-IP overlap
  through the guard's own resolver (covers localhost/127.0.0.1/Compose
  name/DNS name for one server). Every refusal is `ErrForbiddenTarget`
  naming "own internal database", never the DSN.
- Enforced pre-flight in `checkTarget` and at dial time: `guardedDial`
  (MySQL) and `guardedDialAddr` (postgres exact-address) both re-resolve
  and re-compare, so a name rebinding to the installation between check
  and dial is still refused. MySQL carries a driver-scoped target the same
  way even though the install never runs on it. SQLite path identity
  (`InternalPaths`, wal/shm/journal, canonicalisation) unchanged.

Tests (`internal/sqlnode/internal_test.go`, `go test ./internal/sqlnode/...`
green): own-DB refused however spelled per driver (alias host, 0-padded
port, case, options) with no password leak; neighbour-DB (other name,
other port, empty name) passes the guard per driver plus a cross-driver
MySQL-vs-postgres-internal case; rebind-at-dial per driver with
call-count proof; `ParseInternalTarget` table incl. fatal shapes; SQLite
file guard standing beside the network one.

NOT done here — needs integrator (do not expand, per batch non-goals):
- `databaseGuard` in `cmd/kilasflow/main.go` must build
  `Guard{Internal: sqlnode.ParseInternalTarget("postgres", cfg.Database.DSN)}`
  on a postgres install (refuse boot on error), keep `InternalPaths` via
  `database.SQLitePath` on sqlite, and leave `Internal` nil otherwise.
- `make smoke-postgres` live proof and the operator-page prefix/docs
  updates belong to the owners named in the ticket.
- PG-gated tests: run combined suites with `-p 1` (shared-server race).
...
```
 ...
 .pine/tickets/FEAT-a94c8y.md                       |   104 +
 ...
 811 files changed, 173162 insertions(+), 2747 deletions(-)
```
