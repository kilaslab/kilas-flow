---
id: FEAT-wsd0vg
title: extractFromFile for PDF text-layer, TXT, JSON
status: done
priority: high
labels:
    - rag
...
n8n RAG ingest (16706, 3647) extracts text from a downloaded file before the document loader. KilasFlow needed a native `extractFromFile` that reads `Item.Binary` without OCR.

# Acceptance Criteria
- [x] PDF text-layer, UTF-8 text, and JSON operations
- [x] Node description states scanned PDFs need OCR, which this node does not run
- [x] Apache/MIT PDF library (`github.com/ledongthuc/pdf`), not n8n source
- [x] Missing binary storage or property fails closed

# Implementation Plan
Switch on `operation` (pdf/text/json). PDF parser is text-layer only.

# Notes
Support for PDF means a text layer. Image-only scans yield empty text.

# Related Files
- `nodes/extract.go`
- `nodes/extract_test.go`
