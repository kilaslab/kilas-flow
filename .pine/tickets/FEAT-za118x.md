...
## Progress (2026-09-06, APIRefDocs)

Implemented, verified, ready to close. Two deliberate deviations from the plan above, both per Batch-3 assignment:

- Static committed pages instead of `starlight-openapi` + build-time spec. The generator (`scripts/generate-api-reference.mjs`) still boots a real binary via `scripts/openapi-spec.mjs`, but the markdown is committed with a `--check` drift gate (`make generate-api-reference-check`, CI `drift` job) — the `config-reference.go` generate-and-compare pattern. No new docs dependency, no CDN/client JS (air-gap holds), and the docs build stays Node-only so the Go-toolchain trap the plan names does not materialise. Raw spec JSON is never committed (temp file removed in `finally`, like the web/SDK generators).
- Canonical statement lives on the site only. `internal/api` is read-only for this ticket, so the binary `/docs` page is untouched; the site states both sides (overview `reference/api.md`: binary authoritative for the instance, site authoritative for the release, compare via `info.version`).

Also: the "33 operations" count in acceptance is stale — the binary serves 46 (contract page already says 46; generator asserts the live count and fails on any unmapped operation id).

Files (all inside allowed scope):

- NEW `scripts/generate-api-reference.mjs` — groups per contract (9 groups, 46 ops), per-op embed verdicts mirroring `permits()`, `errors.md` (live `ErrorModel`/`ErrorDetail` fields + curated `WorkflowValidationIssue`), `events.md` (asserts all 9 event schemas present + resume/heartbeat/terminal prose), version banner on every page.
- Generated: `docs/src/content/docs/reference/api.md` (overview, replaces stub) + `reference/api/*.md` (9 groups, errors, events).
- Hand-written companion: `reference/api/webhooks.md` (uniform 404, per-trigger auth, filtered-200).
- `reference/api-contract.md`: drift-gates table gains the third detector row. `Makefile`: `generate-api-reference` / `generate-api-reference-check`. CI `drift`: `Docs API reference` step.
- Untouched per constraints: `internal/api/**`, `configuration-reference.md`, `config.example.yaml` (PackLoading owns the current regen; disjoint files, last-regen-wins safe).

Proof: `make generate-api-reference-check` passes fresh; appended probe line → fails naming the fix; regenerated → passes. `make docs-build` green, 39 pages, all internal links valid.
...
```
 ...
 618 files changed, 85381 insertions(+), 2071 deletions(-)
```
