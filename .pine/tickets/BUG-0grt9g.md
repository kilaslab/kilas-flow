...
# Acceptance Criteria
- [x] Start sets a random nonce as a SameSite=Lax HttpOnly cookie, and the nonce is carried inside the signed state. The callback verifies it.
- [x] A state is single-use.
- [x] PKCE (S256) is used.
- [x] Tests cover a callback from another browser, a replay, and a missing verifier.

## Fix (2026-09-25)

Merged in 8ce5ea3 (branch commit 3adcc48). Start sets a per-flow nonce cookie (HttpOnly, SameSite=Lax, Path = callback path, Secure on https, 10 min); the signed state carries SHA-256(nonce); the callback requires the cookie and clears it; a state without a nonce is refused. PKCE S256 with verifier = HMAC(server key, label + nonce), so nothing is stored between start and callback. Single use across replicas via the idempotency_keys table (`ConsumeOnce`, key unclaimable through the header), consumed before the code exchange. No migration. Tests: internal/api/oauth_test.go (fake Google enforcing PKCE, another browser, replay, missing verifier, replay on a second replica), e2e google-oauth.spec.ts. Review: clean.
...
- Commits (3):
  ...
  - `3adcc48e` — BUG-0grt9g: Google Connect completes only in the browser that started it, only once, and only with its PKCE verifier
...
```
 ...
 383 files changed, 50218 insertions(+), 3856 deletions(-)
```
