...
Reference grounded in n8n 2.34.0 (read-only): memory/descriptions.ts (`sessionIdType` fromInput default / customKey, `sessionKey` default `={{ $json.sessionId }}` for fromInput), utils/helpers.ts `getSessionId` (fromInput scopes to `<sessionId>__<sanitized node name>`, charset [A-Za-z0-9_-]; customKey shared verbatim; empty key is a NodeOperationError), MemoryBufferWindow stores `<workflowId>__<sessionId>`. No code copied. Our `SessionKey` (tenant + workflow + session joined by NUL) already covers the workflow scoping.

Done (all in owned files):
- `memoryNode()` gains `sessionIdType` (fromInput|customKey, default fromInput) and `sessionKey` (default `{{ $json.sessionId }}`); legacy `sessionId` kept, no longer statically required. `fixedCollection` check from the ticket: already available (`PropertyFixedCollection`), not needed here.
- `resolveSessionKey`: fromInput -> `<key>__<sanitized node name>` (always; the opt-out IS customKey with the same key in each node, exactly as n8n's scopedSessionHint notice documents — no extra param added); customKey -> verbatim; no stored mode (pre-ticket node) -> legacy `sessionId` verbatim with NO suffix, or a newly-entered `sessionKey` as a defined key. IR carries stored params as-is (no default-fill), so legacy detection is exact.
- Per-node retention: `AgentRequest.SessionPolicy ai.Retention` travels beside the key; `LoopRuntime` uses it via the new `ai.PolicyMemory` interface (`LoadWithPolicy`/`AppendWithPolicy` on BufferMemory), falling back to plain `Memory` for other implementations. `prune` keeps age-then-count order. Zero policy = store defaults via `normalized()`.
- `BufferMemory`: `NewBufferMemory` keeps its signature + variadic `BufferOption`; `WithPerTenantSessionLimit` bounds sessions per tenant (oldest-touched evicted first on write, zero = unbounded); `Stats()` reports total + per-tenant usage; `ForgetWorkflow`/`ForgetTenant` drop scoped sessions. Tenant isolation proven by the pre-existing `TestMemoryIsScopedByTenantAndWorkflow` plus new ceiling/forget tests.
- Key-change behaviour (for release evidence): legacy `sessionId` nodes keep their exact conversation (no migration, no suffix). New fromInput nodes start fresh under suffixed keys — a chat window is a working buffer, not a record of account, per the ticket's recommendation. customKey nodes share verbatim across nodes by design.
- Redaction trap: `executeMemory` resolves from the live `request.Input` item and never reads the persisted record; `session`/`sessionid` are already absent from the redact list (`internal/execution/redact.go`), and `TestMemoryKeepsDistinctBucketsForDistinctSessions` pins the separation. No change needed.
- New tests: per-node retention through the agent (`TestAgentEnforcesPerNodeRetention`: 3 runs x 2 turns, bound 2 keeps 2), policy age-then-count, ceiling eviction oldest-first + `Stats()`, `ForgetWorkflow`/`ForgetTenant`, session modes incl. live-item expression (`TestMemorySessionKeyModes`), validation of both modes.
- `go test -count=1 ./internal/ai/... ./nodes/...` green; `go build ./...` green.

For integrator (outside AI-Cluster ownership, files NOT touched):
- `cmd/kilasflow/main.go`: pass `ai.WithPerTenantSessionLimit(<configured ceiling>)` to `NewBufferMemory` (config value beside the other deployment decisions) and expose `memory.Stats()` where deployment usage is observed.
- Workflow/tenant deletion paths: call `BufferMemory.ForgetWorkflow` / `ForgetTenant` (no callers exist yet — `Forget` itself has none today either).
- Importer: a memory node saved before this ticket keeps `sessionId` and the same conversation automatically (no import action needed); document the fromInput fresh-start scoping in release evidence. Postgres-backed memory stays p6.

## Progress — AIClusterParity integrator follow-up 2026-09-06 (per Main)

- `cmd/kilasflow/main.go`: `NewBufferMemory(ai.Retention{}, nil, ai.WithPerTenantSessionLimit(1000))`; startup log reports `sessionsPerTenant` and current usage. `agentMemory` stays a concrete `*ai.BufferMemory` so composition can observe `Stats()`; a config knob needs an `internal/config` AI section (not touched — PersistenceTier turf), hence the documented constant.
- Workflow delete: `Workflows` handler gained optional `SessionForgetter` + `WithSessionMemory` (same fluent pattern as `WithTriggers`); `Delete` drops the workflow's sessions after the commit, best-effort (a failed drop never fails a completed delete). Wired in `internal/api/routes.go` via new `api.Deps.SessionMemory`, set from `agentMemory` in `main.go`. Covered by `internal/api/handlers/workflows_delete_test.go` (forget called with tenant/workflow; nil store still deletes).
- Tenant delete: no tenant-deletion path exists anywhere in the codebase (verified by grep over handlers/repository) — `ForgetTenant` stays available for the day one lands. Note for the p6 durable tier: it must call the same hook.
- Importer: `memoryToKilas` now writes `sessionIdType` + `sessionKey` under their own names (customKey verbatim; fromInput incl. the `={{ $json.sessionId }}` default materialised as an expression marker), so the scoping rule travels with the key instead of collapsing into the legacy shared bucket. `memoryToN8N` carries the mode back, falling back to customKey-from-legacy-`sessionId` with the original rationale. Pre-ticket KilasFlow nodes (legacy `sessionId`, no mode) are untouched by this: they keep the exact key.
- `Stats()` exposure: exported method + startup log + concrete handle in composition. No new API endpoint: `/health` is documented as a dependency-free liveness probe, and a response-field addition would ripple into API-Contract's OpenAPI assertions mid-flight. A metrics endpoint is future work.
- Verify: `go build ./...` green; `go test -count=1 ./internal/ai/... ./nodes/... ./internal/workflow/...` + interop + handlers green; `gofmt -l` clean. Forbidden paths untouched.
...
```
 ...
 735 files changed, 137515 insertions(+), 2635 deletions(-)
```
