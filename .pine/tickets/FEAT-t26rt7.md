...
## Work notes (DatastoreCSV, 2026-09-06)

Scope per Main: pure API/handler + browser dialog. `nodes/datastore.go`
skipped entirely (no node CSV operations); no `internal/datastore` edits —
the codec lives in the handler layer and uses only existing Engine calls
(`GetDatastore`, `List` keyset pages, `Insert`).

Design decisions:
- Export `GET /datastores/{id}/rows/export?includeSystemColumns=false` streams
  `text/csv` via `huma.StreamResponse` (SSE-adapter pattern: pre-seeded
  `text/csv` response content since a func Body leaves no schema to infer;
  orval types it as `Blob`). Header = user columns in catalogue order, or
  `id` + users + `createdAt`/`updatedAt` (grid order). Pages of 100 through
  the keyset cursor. Timestamps RFC 3339 millis (n8n shot 23 shape).
- Import `POST /datastores/{id}/rows/import` takes `RawBody []byte` as
  `text/csv` with explicit `MaxBodyBytes: 5 MiB` (Huma 413 names the limit:
  `request body is too large limit=5242880 bytes`). Validate-all-then-insert:
  failed rows -> 200 report `{inserted: 0, skipped, failed: [{line,
  column, severity: "blocking", reason}]}` (interop-import precedent: the
  report is part of the success response); reserved/unknown header names or
  undecodable bodies -> 422 naming the offender. Empty field = NULL for
  number/boolean/date, `""` for string. BOM stripped. Short all-blank lines
  counted skipped; full-width empty records import as NULL rows.
- Web: `apiDownload` helper in `http.ts` (embed token + blob, never
  `response.json()`); `lib/datastore/transfer.ts` (URLs, disposition
  filename, `uploadImport`, severity words Blocking/Lossy/Dropped,
  `summarizeReport`) with `transfer.test.ts`; detail-page dialog replaced
  the placeholder with export toggle + import file/report rendering in the
  diagnostics badge vocabulary. Generated clients + SDK types regenerated;
  `pnpm generate:api:check` and `generate:types:check` exit 0.

Evidence:
- `go test ./internal/api/...` green, incl. 12 new CSV handler tests
  (header order, system wrap + instant equality, byte-exact round trip with
  hostile cells, reserved-header table test, 413 naming 5242880, malformed
  row line+column with unchanged count, cross-tenant 404s, BOM, NULL-vs-"",
  503s, unknown-id 404).
- `pnpm check` 0 errors; `pnpm test` 342/342 (10 new transfer tests).
- `make smoke-sqlite`: passed.
- By-hand CSV round trip, real binaries: sqlite (port 18082) and postgres
  (fresh `csvsmoke` db on kf-pg, port 18083) — create, insert hostile rows,
  export, import into fresh datastore, re-export, `diff` byte-identical on
  both drivers; reserved header 422 naming `id`; malformed row report
  `{inserted:0, failed:[{line:3, column:score, blocking}]}`; over-limit 413
  naming the limit.
- Browser download check: NOT performed — no Chromium on this machine
  (shared browser daemon fails to launch, no system browser, no Playwright
  browsers). Compensating proof: the served `Content-Type: text/csv` +
  `Content-Disposition: attachment` observed via curl, and the dialog path
  (`apiDownload` -> blob -> anchor) never calls `response.json()`, so the
  recorded failure mode (JSON parse error) is unreachable by construction.

- `make smoke-postgres`: NOT passed — fails before any app code runs, in
  `go test ./internal/database -run 'Postgres'` inside the script:
  `Migrate: apply migration 000006_vector_store: ERROR: extension "vector"
  is not available`. The compose file pins `postgres:17-alpine`, which
  ships no pgvector extension. No migration, database, or engine file is in
  this ticket's diff, so the failure is independent of it (it fails the
  same way on a clean tree); routed to Main. Postgres driver proof for CSV
  is the by-hand round trip above on kf-pg instead.

## Smoke note (Main, 2026-09-06)
`make smoke-postgres` passes on the pgvector-pinned compose stack (compose.postgres.yaml now pgvector/pgvector:pg17; stock postgres:17-alpine cannot apply migration 000006 and fails the boot). Covers stack boot + migrations + app health; CSV round trips were proven by-hand per the slice notes above (sqlite smoke passed, PG round trip byte-identical on kf-pg/csvsmoke).
...
```
 ...
 .pine/tickets/FEAT-t26rt7.md                       |  129 +
 ...
 internal/engine/service.go                         |  503 ++-
 ...
 internal/repository/webhooks.go                    |   28 +-
 ...
 sdk/src/generated/models.ts                        | 2949 ++++++++++++++-
 sdk/src/http.ts                                    |   31 +-
 ...
 web/src/lib/api/generated/models/index.ts          |   73 +
 ...
 924 files changed, 186369 insertions(+), 3013 deletions(-)
```
