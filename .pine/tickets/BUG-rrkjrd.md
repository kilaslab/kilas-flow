...
- Added `TestDollarItemResolvesThroughHttpAndALoop`: Manual -> Split Out -> HTTP Request (real stub, one request per item) -> Loop Over Items (batch 1) -> Set body -> done -> probe reading `$('Split Out').item.json.v` resolves a/b/c per item, which is the HTTP Request and Loop Over Items half of the acceptance list.
...
- Commits (4):
  - `5ec58bd2` — BUG-rrkjrd: lineage through HTTP Request and Loop Over Items — expression/engine
  - `c0bd6589` — BUG-rrkjrd: record end-to-end lineage verification — expression/engine
  - `d4e1e1f7` — BUG-rrkjrd: per-item paired lineage for $('X').item — expression/engine
...
```
 ...
 434 files changed, 69245 insertions(+), 4725 deletions(-)
```
