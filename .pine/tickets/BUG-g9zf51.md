...
# Acceptance Criteria
- [x] One helper, for example in safehttp, rewrites any `*url.Error` to scheme://host with the path and query withheld. Every outbound call site above uses it.
- [x] Node errors and error items are scrubbed of the resolved credential's secret values before they are persisted or emitted. `ScrubResolved` exists and nothing calls it.
- [x] Tests reproduce both live cases and assert that no secret reaches the execution record, the error item or the log.
- [x] BUG-asdh5q, the lifecycle path, is closed by the same change.

## Fix (2026-09-25)

Merged in 28e1da6 (branch commit c8dd274). `safehttp.RedactError`/`RedactURL` rewrite any `*url.Error` (nested or wrapped) to scheme://host:port, keeping the operation and cause (`errors.Is` still finds ErrBlocked/deadline), at every outbound call site: HTTP node, routing (call + download), loadoptions, wasm packs, sidecar egress, MCP, OpenAI-compatible, pgvector, Google, Telegram download/lifecycle/polling, pack media download, Vault, request lifecycles. The engine also scrubs the resolved credential's secret values (trimmed, query- and path-escaped forms, httpCustomAuth template strings) from node errors, per-item outcomes, batch failures and error items per attempt (`internal/engine/secret_scrub.go`, `credentials.SecretValues`/`ScrubText`). Tests reproduce both live cases (httpQueryAuth closed port, Telegram baseUrl closed port) end to end through the stored execution record. Review: clean.
...
```
 ...
 417 files changed, 52799 insertions(+), 3986 deletions(-)
```
