...
## Progress (2026-09-06, LangChainImport)

- Read reference checkout (n8n 2.34.0, read-only, format facts only) + design-refs/n8n-v2/INDEX.md + memory/licensing.md. No source copied; facts transcribed to testdata.
- 6 of 8 mappings already in HEAD (agent, chainLlm, lmChatOpenAi, lmChatOpenRouter, memoryBufferWindow, toolHttpRequest); importConnections is kind-generic via KnownConnectionKind (ai_outputParser/ai_tool already declared in document.go), so no edge changes needed.
- Verified format facts: toolWorkflow v1 [1,1.1,1.2,1.3] + v2 [2,2.1,2.2], outputs ai_tool, params name/description/source(database|parameter)/workflowId/workflowInputs(resourceMapper)/workflowJson; outputParserStructured [1,1.1,1.2,1.3], outputs ai_outputParser, schemaType fromJson|manual (from 1.2), example vs manual keys, autoFix/customizeRetryPrompt/prompt.
- Extended testdata/n8n_cluster_nodes.json with both entries; anti-drift test now (correctly) fails until mappings land.
- Waiting on AIFamily (native kilasflow.workflowTool + kilasflow.outputParser parameter contracts) before writing converters; tests use the real registry so kilasTypes must exist first.

## Progress (2026-09-06, LangChainImport, cont.)

- Added mappings `@n8n/n8n-nodes-langchain.toolWorkflow ↔ kilasflow.workflowTool` (export 2.2) and `outputParserStructured ↔ kilasflow.outputParser` (export 1.3); decimals per house rule; exact-match lookup untouched.
- Converters per AIFamily-adopted contract: tool carries toolName/toolDescription/workflowId/workflowInputs(resourceMapper entries, expression-translated); source=parameter refused blocking on workflowJson; v1 fields/responsePropertyName dropped-named. Parser translates schemaType fromJson/manual→exampleJson/jsonSchema (≤1.1 jsonSchema→jsonSchema); autoFix→maxRetries 2/0; customizeRetryPrompt/prompt dropped-named (only when auto-fix armed).
- Import reconciles hasOutputParser flags against surviving ai_outputParser edges (dropResolvedParserFlags); agent/chain wording updated ("imports as its own output-parser node").
- Tests: full-cluster fixture (agent+chain, 2 models, memory, 2 tools, parser) asserting params/edges/reconciliation/lossy; round-trip incl. versions 2.2/1.3; inline-tool refusal; pre-1.2 parser.
- State: package green except TestImportMapsTheWholeLangChainCluster + TestLangChainClusterRoundTrip edge assertions, held back until AIFamily registers kilasflow.workflowTool/kilasflow.outputParser with ai_tool/ai_outputParser ports. No edits outside internal/interop/n8n (+ ticket notes).

## Done (2026-09-06, LangChainImport)

- `go test ./internal/interop/...` green (n8n + corpus), gofmt clean.
- New: toolWorkflow + outputParserStructured mappings/converters/exports; hasOutputParser edge reconciliation; full-cluster fixture (agent+chain, 2 models, memory, 2 tools, parser) with param/edge/reconciliation/lossy assertions; round-trip versions 2.2/1.3; inline-tool refusal; pre-1.2 parser; cluster added to TestImportedWorkflowCompiles (credential-bound like the existing cluster test).
- Two cross-agent fixes found via the compile gate: maxRetries emitted as float64 (native validator takes JSON numbers); natives registered by AIFamily.
- Files: internal/interop/n8n/{n8n.go, parameters.go, n8n_test.go, testdata/n8n_cluster_nodes.json} + ticket notes only.
...
```
 ...
 762 files changed, 150862 insertions(+), 2676 deletions(-)
```
