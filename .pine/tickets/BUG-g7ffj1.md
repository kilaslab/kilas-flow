...
Closed by `pine close --evidence` on 2026-09-23. Rewritten on 2026-09-23 to list only this ticket's own commits (`git log --grep "BUG-g7ffj1" 2f8e9b1..5830b4e`) and the files exactly those commits changed.

- Range: `2f8e9b1..5830b4e`
- Commits (3):
  - `6ff3af6a` — BUG-g7ffj1: ClaimDue's three deactivate-and-continue sites share one closure
  - `a0e54452` — chore(pine): close BUG-g7ffj1 with its landing evidence
  - `49cd5883` — BUG-g7ffj1: an impossible cron is refused, not fired on every tick
- Merged by (1):
...
```
 .pine/tickets/BUG-g7ffj1.md                       | 306 +++++++++++++++++++++-
 internal/api/handlers/workflows.go                |   7 +
 internal/api/schedules_test.go                    |  28 ++
 internal/repository/schedules.go                  |  51 ++++-
 internal/scheduler/scheduler.go                   |  51 +++--
 internal/scheduler/scheduler_test.go              |  83 ++++++-
 web/src/lib/workflow-editor/execution.test.ts     |  19 ++
 web/src/lib/workflow-editor/execution.ts          |  10 +-
 web/src/routes/(dashboard)/schedules/+page.svelte |   2 +-
 9 files changed, 523 insertions(+), 34 deletions(-)
```
