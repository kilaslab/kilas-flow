...
# Acceptance Criteria
- [x] Default domain scopes: `openAiApi` → api.openai.com; `openRouterApi` → openrouter.ai; `telegramApi` → api.telegram.org. Check for other fixed-host types too.
- [x] The compiler refuses a credential type the node does not declare.
- [x] Embed sessions and scoped keys may not run a document that attaches an unscoped credential, or document confinement is bound to the node type and target host. `EmbedScopeIssues` applies to scoped keys as well.
- [x] Tests cover each exploit path.

## Fix (2026-09-25)

Merged (branch commits 292a76e, 698c3eb, 8d053eb). Credential types declare DefaultDomains / DefaultDomainsFrom / NeverSentOverHTTP (internal/credentials/scope.go): openAiApi → api.openai.com, openRouterApi → openrouter.ai, Google keeps its list, telegramApi and wahaApi follow their own stored baseUrl host (fallback api.telegram.org); defaults apply at check time, so existing empty-list credentials are covered, and `RedirectScope().Unbounded` follows the effective domains. The compiler refuses a credential type a node does not declare (disabled nodes exempt). Embed sessions and scoped API keys may not create/save/publish/restore/run/retry a document attaching an unscoped credential to a node that calls out (database/SQLite/JWT and webhook/form inbound-auth credentials exempt; disabled nodes exempt; the embed grant check still applies to disabled nodes); the test endpoint's stored-scope intersection uses the effective scope. Behaviour change documented: an OpenAI/OpenRouter key used through a gateway baseUrl needs the gateway host added. Tests: scope_test.go, authenticate_test.go, ai_scope_test.go, credentialscope_test.go, compiler_test.go, credential_confinement_test.go. Review round 1 found disabled nodes not exempt (fixed); round 2 (merge with main): clean.
...
- Commits (5):
  ...
  - `8d053eb1` — BUG-xkz7qx: an unscoped credential on a disabled node no longer blocks an embed session or a scoped key
  - `698c3ebf` — merge: main into BUG-xkz7qx and BUG-a7p6c8
  - `292a76e1` — BUG-xkz7qx: an embed guest or a narrowed agent token can no longer send a granted credential to a host of its own
...
```
 ...
 446 files changed, 56599 insertions(+), 4100 deletions(-)
```
