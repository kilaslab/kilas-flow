...
- Commits (11):
  - `aefa4ff6` — FEAT-zjrw76: a constructor that calls RegExp() plainly gets a regular expression, so Luxon's DateTime.fromFormat works
  - `2c9dd164` — merge: FEAT-zjrw76 Intl, locale formatting and Luxon lane
  - `3a051e1c` — merge: FEAT-zjrw76 crypto lane (Node's crypto, done by Go)
  - `244a7707` — FEAT-zjrw76: the replaced locale methods keep the name, length and attributes of the built-ins they replace
  - `4371a542` — FEAT-zjrw76: BigInts format exactly however long, and a long numeric string reaches infinity as in Node rather than NaN
  - `3d472e77` — FEAT-zjrw76: Code-node JavaScript gets Intl, locale-aware formatting and Luxon, held to recorded Node 24 goldens
  - `68192e04` — FEAT-zjrw76: Node's crypto for Code-node JavaScript, done by Go
  - `e64136e1` — FEAT-zjrw76: Buffer with every Node encoding, WHATWG URL, the web globals and timers
  - `9c9ca6de` — FEAT-zjrw76: the byte encodings Node names, as natives for Buffer and crypto
  - `f2632a3d` — FEAT-zjrw76: the runtime's own modules, require's allowlist, and libraries that leave no global
...
```
 ...
 394 files changed, 40771 insertions(+), 783 deletions(-)
```
