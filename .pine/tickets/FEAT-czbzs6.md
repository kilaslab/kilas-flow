...
## Progress (PackLoading, 2026-09-06)

- Loader: `internal/nodepack/loaddir.go` — `DirDeps` (mirrors waha.Deps) +
- `LoadDir(deps, dir)`. One subdirectory per pack with `pack.json` + `pack.sha256`
- (`sha256sum` output compatible). Empty/absent dir is a silent no-op; every
- other failure names the pack and the reason. Trigger packs also go through
- `RegisterTrigger`, mirroring `waha.Register`, so disk-loaded packs register
- exactly what the same bytes embedded register.
- Config: `packs` single-word section, `dir` key, default `""`, env
- `KILASFLOW_PACKS_DIR` (verified reachable via one-off env test, then removed).
- Regen `go run ./scripts/config-reference.go` done — diff is packs-only;
- `--check` passes. APIRefDocs: safe to re-run generator now (last-regen-wins).
- Wiring: `cmd/kilasflow/main.go` pack-loading lines only — `nodepack.LoadDir`
- after `waha.Register`, errors wrapped as `load directory node packs`.
- Tests: `go test ./internal/nodepack/...` green — dir copy of telegram pack
- loads with `SourcePack` + runs sendMessage against a stub; WAHA trigger pack
- from disk registers definition + event table; bad manifest / checksum
- mismatch / missing checksum / missing manifest / `kilasflow.` prefix /
- duplicate all fail naming the pack; empty config is silent + registers nothing.
- Out of scope kept out: hot reload, remote install, authoring toolchain.
...
```
 ...
 .pine/tickets/FEAT-yx0qt6.md                       |   43 +-
 ...
 618 files changed, 84111 insertions(+), 2071 deletions(-)
```
