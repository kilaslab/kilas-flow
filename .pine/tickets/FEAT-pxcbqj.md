...
- Commits (15):
  ...
  - `fc3e48cb` — FEAT-pxcbqj: e2e: crypto, Luxon and Intl in a live run
  - `9d20e7a3` — FEAT-pxcbqj: a Python refusal points at pythonCode, and the heap ceiling is labelled in MiB
  - `6765798d` — FEAT-pxcbqj: per-item Code nodes that continue on failure go on past the failed item, as n8n's do
  - `71b540c7` — merge: FEAT-pxcbqj docs, skills and catalogue lane
  - `33fc5460` — FEAT-pxcbqj: notes: the worker choice and the binary size so far
  - `7464aa19` — FEAT-pxcbqj: docs: nothing still says this server does not run JavaScript
  - `530485fd` — FEAT-pxcbqj: e2e: live workflows wire the Code (JavaScript) node to the rest of the product
  - `68ee6301` — FEAT-pxcbqj: e2e: a Code node that turns one item into several keeps that item as their source
  - `6267e610` — FEAT-pxcbqj: docs: the picker offers Code (JavaScript) and keeps the placeholder out
  - `b97b6b44` — FEAT-pxcbqj: docs: the skills know a JavaScript Code node runs
  - `aa52e0f0` — FEAT-pxcbqj: docs: JavaScript Code nodes run, and where they run
...
```
 ...
 394 files changed, 41300 insertions(+), 783 deletions(-)
```
