...
## Progress notes (MCPNode, 2026-09-06)

- Implemented in `nodes/ai.go` (MCP section only) + new `nodes/ai_mcp_test.go` (9 tests, all green; `go test ./nodes/ ./internal/ai/...` green).
- No `internal/mcp` package and no `go-sdk` dependency: the SDK would need a licensing review first, and my ownership covers `nodes/ai.go` only, so the client is a small JSON-RPC Streamable HTTP implementation inside `ai.go` (initialize, notifications/initialized, tools/list, tools/call; plain JSON and text/event-stream responses). Every request goes through `safehttp.NewClient(policy)` and `request.Authenticate`, so SSRF policy, credential scoping and domain checks match the HTTP node.
- No new credential type: the node accepts the existing `httpBearerAuth`/`httpHeaderAuth`/`httpBasicAuth`. Only the credential id travels in the descriptor; a test asserts the secret never appears in the execution output.
- Tool names are the server's own names verbatim (MCP already constrains the charset); the canvas name travels as `nodeName`, so duplicate detection names both nodes. No per-node toolName/toolDescription params — descriptions and schemas come from the server.
- Session lifetime: stateless fresh handshake per tool call (no shared session cache). Noted debt if the extra round trips ever matter.
- Validator and executor both refuse `stdio` URLs with an explicit diagnostic.
- Integration left for Main (outside my ownership): add `mcpClientToolNode(),` to `RegisterAll` in `nodes/core.go`, `MCPClientToolExecutorID: NewMCPClientToolExecutor(httpPolicy),` to `RegisterExecutors` in `nodes/executors.go`, and extend `TestToolFamilyDefinitions` with `singleToolPort(t, nodes.MCPClientToolNodeType)`. No `node-visual.ts` change needed (`builtin:wrench` already exists).
- Pre-existing gap (not introduced here): the compiler never calls `CheckDuplicateToolNames` — enforcement is the agent's runtime fail-fast. Wiring the compiler check is a separate ticket.
...
```
 ...
 .pine/tickets/FEAT-c2a081.md                       |    66 +
 ...
 811 files changed, 168433 insertions(+), 2747 deletions(-)
```
