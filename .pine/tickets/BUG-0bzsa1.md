...
# Acceptance Criteria
- [x] With a credential scope present and an empty domain list, a redirect may only follow to the same host as the first request.
- [x] A scheme downgrade is refused while a credential is attached.
- [x] Lifecycle calls apply credentials through the same path the engine uses: host check, redirect scope, type check.
- [x] Tests cover a cross-host redirect for header, custom and WAHA credentials, and a lifecycle call to a disallowed host.

## Fix (2026-09-25)

Merged in 28e1da6 (branch commits de20fea, 0e7b9b1). A credential with no domains follows a redirect only to the first request's host; no https→http step while a credential is attached; `Record.RedirectScope()` builds every scope. Lifecycle calls, Telegram lifecycle/polling/download apply credentials through the engine's path (`ScopeRequest` host check + scope, `CheckType`). Review round 1 found the AI chat model and embeddings never attached a scope (same-host downgrade would resend the Bearer key); round 2 fix scopes the chat model (every attempt, Complete and Stream), embeddings/vector store, Vault `X-Vault-Token` and the Google token exchange/refresh POST body, with real TLS→plain same-host redirect tests. Review round 2: clean.
...
- Commits (4):
  ...
  - `0e7b9b15` — BUG-0bzsa1: the AI chat model, embeddings, Vault and OAuth token calls carry a redirect scope
  - `de20feaf` — BUG-0bzsa1: a credential's header stays on its first host across redirects, and lifecycle calls apply the engine's checks
...
```
 ...
 417 files changed, 53784 insertions(+), 3988 deletions(-)
```
