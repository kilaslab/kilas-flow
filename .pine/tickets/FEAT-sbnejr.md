...
- internal/ai/outputschema.go (new): ParseOutputSchema (jsonSchema|exampleJson, path-named shape errors, explicit maxRetries 0 disables), ValidateValue subset (type/required/properties/items/enum), ParseAndValidateOutput (fence-strip, error carries validation + raw text).
- agent.go: AgentRequest.OutputSchema/OutputMaxRetries; synthetic format_final_json_response defined-not-invocable; invalid format calls + plain-text fallback retried bounded, then fail diagnosably; ai.tool.* events throughout. ai_outputParser kind already in document.go (all 13 present) — no change needed.
- nodes/ai.go: kilasflow.outputParser (single ai_outputParser output); agent + chain accept capped parser slot (compiler kind check refuses miswiring); agent payload carries parsed object; chain retries with feedback then fails with error+raw.
- document.go needed NO change (ai_outputParser already known). Same RegisterAll dependency as je4f4t.
...
```
 ...
 762 files changed, 150086 insertions(+), 2676 deletions(-)
```
