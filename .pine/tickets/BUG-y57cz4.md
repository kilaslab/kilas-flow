...
- Commits (11):
  ...
  - `020924c4` — BUG-y57cz4: classify a pointer error by the type behind it, and fix the drop in its test
  - `9364cb0c` — chore(pine): record the security-review remediation evidence on 9853ay, 8dmp5y, y57cz4, fv5fer
  - `d3312eba` — BUG-y57cz4: the ignored-key warning reaches the configured log stream — config
  - `67e97ff3` — BUG-y57cz4: a store failure is a logged 500, not a 422 carrying the driver's text — api handlers
  - `9acdd7ae` — chore(pine): reopen 9853ay, 8dmp5y, y57cz4, fv5fer with the security review's findings
  - `d0f26a19` — chore(pine): close the 53 EPIC-cfe7ny remediation tickets with work evidence
  - `f575d736` — BUG-y57cz4: nodes name the shared binary-storage error so it points at the setting — http/telegram
...
```
 ...
 450 files changed, 82280 insertions(+), 4933 deletions(-)
```
