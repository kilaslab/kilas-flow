...
- `$KILASFLOW_N8N_REFERENCE/packages/workflow/src/constants.ts:54-55` — `DATA_TABLE_NODE_TYPE` and `DATA_TABLE_TOOL_NODE_TYPE`, both real type strings in exported JSON.
- `$KILASFLOW_N8N_REFERENCE/packages/workflow/src/data-table.types.ts` — the row and table operation unions, the `DataTableFilter` shape and the system columns; the node's parameter descriptions are absent from this sparse checkout.
...
## Progress (2026-09-06, DatastoreImport)

- Landed both mappings, interop only: `n8n-nodes-base.dataTable ↔ kilasflow.datastore`
  and `n8n-nodes-base.dataTableTool ↔ kilasflow.datastoreTool` (export pin 1/1;
  decimals per house rule; exact-match lookup untouched). Tool kilasType per
  DatastoreAgentTool (`kilasflow.datastoreTool`, standard `tool`/ai_tool port).
- Converters carry all 12 ops (7 row incl. deleteRows→delete and
  rowExists/rowNotExists→ifExists/ifNotExists; 5 table incl. update→rename and
  delete→deleteTable), all three locator modes (list/name/id + cachedResultName),
  both mapping modes (defineBelow/autoMapInputData verbatim, expression-valued
  entries translated, schema copy kept), and the filter panel under the NODE
  paths keyName/condition/keyValue (default eq silent; unknown condition falls
  back to eq named; expression-valued keyName refused by name).
- Every carried table reference reports blocking naming the n8n id and cached
  name; nothing is created as a side effect and no name-matching bind is
  attempted. Empty locator for a table-needing op is blocking; create/list
  without a table slot get the editor's empty locator so they compile.
- Export restores n8n op names and asserts the connection count; the datastore
  identifier is reported lossy in credential-reference terms.
- Corpus: authored `corpus/fixtures/control-datatable.json` (create → list →
  get-by-name with filter → rename). Scoreboard wired with a per-test SQLite
  store (`nodes.WithDatastoreEngine`) + `corpus` tenant so the control reaches
  runnable; no other fixture moves (inventory only +dataTable/+manualTrigger).
- Baseline rescored: total 39→40, imported 39→40, activatable 13→14,
  runnable 3→4 — the +1 in every tier is the new control.
- `go test ./internal/interop/...` green (n8n + corpus).
- Files: internal/interop/n8n/{n8n.go, parameters.go, n8n_test.go,
  corpus/fixtures/control-datatable.json, corpus/scoreboard_test.go,
  corpus/baseline.json, corpus/BASELINE.md} only.
...
```
 ...
 902 files changed, 179840 insertions(+), 3012 deletions(-)
```
