...
## Spike outcome (2026-09-06, MAFSpike)

Verdict: ADOPT as an optional runtime. LoopRuntime stays the default; no
`cmd/` change in this ticket. Three pass criteria, each against the
framework's own code at v0.1.0 (module cache, not docs):

1. Injectable `*http.Client` — PASS. `NewChatCompletionsAgent(oclient
   openai.Client, ...)` (`provider/openaiprovider/chat.go:76`; same shape at
   `responses.go:35,40`) takes a caller-built SDK client, and openai-go
   accepts it via `option.WithHTTPClient(*http.Client)`, so the
   safehttp-derived transport (SSRF dialer + per-credential scoping) is
   preserved. Their own test injects a custom client
   (`responses_test.go:6876-6881`).
2. Streaming deltas — PASS. `agent.Stream(true)` (`agent/options.go:119`);
   the streaming branch yields one `*ResponseUpdate` per chunk with
   `TextContent` deltas; `ResponseStream` is `iter.Seq2`, consumed live.
3. Per-turn token usage — PASS. `addUsage` runs on both paths (streaming sets
   `StreamOptions.IncludeUsage=true` because OpenAI omits usage otherwise);
   `Response.Usage()` aggregates, `ResponseUpdate.Usage()` reads
   `UsageContent`.

Licence: MIT (module LICENSE), compatible with the Apache-2.0 root — no
licence change, no new exception. No n8n contact of any kind.

Weight: `go.mod` gains one direct require plus four indirect
(cespare/xxhash, google/jsonschema-go, otel x2); the heavy providers
(Azure, Anthropic, MCP, gRPC) are in the module graph but NOT the build
graph — maf imports only agent/harness/tool/message/functool.
`CGO_ENABLED=0 go build ./internal/ai/...` works.

Proof: `internal/ai/maf/runtime_test.go::TestTwoTurnToolLoop` — real
framework loop (stub provider Run + toolautocall middleware + typed
functool): turn 1 calls get_weather(Berlin), turn 2 answers; asserts tool
args, output text, `ToolCalls: 1`, `Iterations: 2`, exact usage mapping
(120/30/150), non-empty messages, and `EventModelDelta` emission.
`go build ./...` + `go test ./internal/ai/...` green.

NOT in this ticket (adoption remainder): ai.Tool → framework-tool mapping,
ai.Memory → HistoryProvider, per-request iteration bound (bound currently
lives in toolautocall config on the prebuilt agent), config-selectable
wiring in `cmd/`, and the parity fixture proving identical output shape.
`internal/ai/maf/doc.go` corrected (stale pseudo-version comment replaced).
...
```
 ...
 762 files changed, 151638 insertions(+), 2676 deletions(-)
```
