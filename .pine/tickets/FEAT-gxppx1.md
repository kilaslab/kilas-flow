...
Status: implemented (runner core), awaiting Main verification. `fleet.go`
only; no new migration files, no lease columns (would need migrations/
outside this owner), no Ready/cmd wiring (siblings' files).

Changed:
- `Run` re-reads the work list after every datastore (one behind-row per
  iteration) instead of snapshotting once; per-datastore transactions with
  the stamp sharing the step's transaction (step-once across reruns);
  ahead-guard refuses datastores past CurrentSchemaVersion naming id + both
  versions; ctx honoured between datastores. SQLite bound stays the step
  author's contract (chunked backfills), documented on FleetRunner.
- New `VersionSpread` query (version->count, no physical tables opened) for
  the future readiness handler to call.
- Row ops (`rows.go` `gatedLookup`) refuse behind datastores until the
  runner reaches them, naming both versions.

Deliberate deferrals (need files outside internal/datastore): Ready Fetcher
version spread, post-server fleet scheduling in cmd, lease/expiry claim
columns, Ready handler test, smoke runs under make targets.

Tests (`migrate_test.go`, sqlite + PG16 kf-pg-vector:55434 green):
- mid-run create picked up by the same run (step creates b-behind through
  its own tx handle — a second transaction would deadlock SQLite's single
  connection), born-at-current asserted, second run opens nothing (step
  count stays 2, migrated 0);
- kill inside step leaves version 0 (rollback), resume reaches current;
- ahead version refused naming both; mixed-fleet spread map[0:1 1:2].
Found while testing: GORM `default:1` turns zero SchemaVersion into the DB
default on Create — tests park behind via explicit UPDATE (same trap the
step-once design avoids by stamping inside the step tx).
...
```
 ...
 785 files changed, 138950 insertions(+), 2844 deletions(-)
```
