...
## Notes (NodeCoverage, 2026-09-06)

Delivered `e2e/tests/node-coverage.spec.ts` (new file only; no harness edits).
`pnpm playwright test tests/node-coverage.spec.ts` → 13 passed; smoke still 5 passed.

Live catalogue at time of writing: 47 entries on GET /api/v1/node-types —
42 builtin (incl. kilasflow.unsupported at arities/versions 1, 2, 4, 8 and
postgres/mysql at versions 1 and 2) + 5 pack (pack.telegram@1,
pack.waha@202409/202502, pack.wahaTrigger@202409/202502). No sidecar or
datastore entries yet; no entry carried `unavailable`.

Coverage split: 31 entries run to terminal success against the loopback stub
(all pure nodes, httpRequest, sqlite via server.dataDir, code, dateTime,
1-second wait, loop with a wired-back body, all four triggers incl. both WAHA
trigger versions, sticky note, executeWorkflow after publish, pack.telegram
and both pack.waha action versions); 16 entries covered at the validation
tier (draft saves with 201, run refused with 422 + named message):
foreignCode + unsupported x4 (refusal is the correct behaviour), the 7-node
AI cluster (agent/chainLlm name the missing Chat Model port; chatModel and
both providers name their credential; memory names the session key; httpTool
names toolDescription), postgres/mysql x4 (missing-credential picker message;
SQLite covers the SQL path that runs headless).

Tier decision (per the ticket's own three-tier recommendation): nodes needing
a third party stay at tier 1 (configured + validated) in this suite —
the AI cluster needs a live LLM endpoint (the fixed harness stub cannot serve
the model protocol, and disconnected AI sub-nodes are rejected at save with
`workflow.invalid_topology`), remote databases need a live database service.
Tier 3 (real service) belongs to the capstone suite.

Heads-up for Main/sequencing: the report test fails on any catalogue entry
with neither a case nor an exclusion, by design. If the Datastore node lands
before test-e2e runs, the suite will flag it until a case is added
(DatastoreEngine was asked for its type string + minimal headless params).
Same for any future node type or version.

## Notes (NodeCoverage, 2026-09-06, +4 AIFamily types)

Catalogue grew 47 → 51 (all builtin): kilasflow.calculator@1 runs to success
(expression `7 * 6` → `{result: 42}`, no network/credentials), while
kilasflow.calculatorTool@1, kilasflow.workflowTool@1 and
kilasflow.outputParser@1 join the validation tier — agent-side sub-nodes with
no main ports, so the draft saves and the run names the missing
configuration (`toolDescription is required` for both tools,
`jsonSchema is required when the schema type is JSON Schema` for the parser),
mirroring the existing httpTool case. kilasflow.chainLlm stays validation
tier as instructed. Split is now 32 run + 19 validation = 51, exclusions
still none. Spec 13 passed, smoke 5 passed.
