...
- [ ] `workflow.Item` carries provenance identifying the source node, its run index, and the index of the input item this item descends from, and it round-trips through JSON persistence unchanged.
- [ ] Every built-in executor that maps items one-to-one propagates provenance; `executeMerge` preserves the provenance of each side rather than flattening it.
- [ ] The runner records each node's output per run index, so a node that ran twice has two retrievable output sets rather than one that overwrote the other.
- [ ] `execution_node_runs` records a run index distinct from `Attempt`, and a node that ran more than once in an execution produces more than one row without violating a unique index.
- [ ] Given `A → Set → B` over three items, an expression evaluated on item 2 of `B` that reaches back to `A` resolves to item 2 of `A`, not item 1.
- [ ] When lineage genuinely cannot be established — after a Merge that combined unrelated streams, or after a node that produced a different item count — the lookup fails with a message naming the node and the reason, rather than returning the first item.
