...
## Progress (DatastoreRows, 2026-09-06)

Status: implemented, awaiting Main verification. All work inside
`internal/datastore/*` (+tests); engine DDL untouched, `doc.go` closing
paragraph refreshed.

Added:
- `filter.go` — service shape `{type: and|or, filters: [{columnName,
  condition, value}]}` plus `NodeConditionsToFilter` mapping the NODE names
  (`filters.conditions[i].keyName`/`.keyValue`, Must Match Any/All) to the
  service names; operator set `eq, neq, like, ilike, gt, gte, lt, lte,
  isEmpty, isNotEmpty` through ONE `switch` (`conditionFragment`) with a
  refusal naming the set. `like` is case-sensitive on both drivers (GLOB on
  SQLite, LIKE on PG); `ilike` insensitive (LIKE on SQLite, ILIKE on PG).
  Keyset cursor `row-v1\0<id>` (literals `cm93LXYxADE`=1, `cm93LXYxADQy`=42
  pinned in test); `DefaultRowPageSize=25`, `MaxRowPageSize=100`, ReturnAll
  bypasses limit; LimitPerInputRow is a node concern (one List per input).
- `rows.go` — Insert/Get/List/Update/Upsert/Delete/Clear. Reads project the
  catalogue's live columns (never SELECT *); columns resolve against the
  catalogue before quoting (system id/createdAt/updatedAt allowed,
  dryRunState refused as reserved); values bind as `?` only. Dry run is
  read + computed after-image (no rolled-back write: SQLite is single-conn).
  Update bumps `updatedAt` database-side. Upsert merges filter eq-keys into
  the insert; read-then-write documented non-atomic (locking is V2-p9-6).
  Row ops refuse ahead/behind versions naming both versions, siblings keep
  serving.

Deliberate deviation (batch ownership): the executions-cursor extraction to
`internal/repository/cursor.go` and the smoke runs under make targets were
out of scope for this owner (repository/Makefile belong to siblings). The
row cursor lives in-package with a recorded-literal test instead, so no
second copy of the executions format exists.

Tests (`go test ./internal/datastore/...` green):
- sqlite: full suite green. PG16 on kf-pg-vector:55434: full suite green
  incl. like/ilike agreement (`%app%` like=[2], ilike=[1 2 3] on both),
  interleaved-insert keyset stability, dry-run byte-identity (counts +
  canonical dumps), unknown-column-no-SQL, placeholder-only statements.
  NOTE: plain kf-pg:55433 lacks the `vector` ext so 000006 fails there —
  use kf-pg-vector.
- PG rerun note: the shared `kilasflow` db on kf-pg-vector got polluted
  mid-session by sibling vector work (`vector_collections` outside goose
  tracking breaks every PG Migrate, datastore AND database packages). Final
  green PG run used a clean `kilasflow_ds` database on the same server
  (created via docker exec, `vector` ext enabled), DSN
  `postgres://kilas:hunter2@127.0.0.1:55434/kilasflow_ds?sslmode=disable`
  — left in place for re-verification. 12/12 postgres subtests PASS, like
  ([2]) and ilike ([1 2 3]) identical on both drivers.
- Combined PG run `go test -p 1 ./internal/datastore/...` green; keep `-p 1`
  for combined PG runs (shared-server resets race, per FEAT-ss44d9 note).
  `internal/database` PG failure (`TestRollingBack...`: leftover
  `vector_collections` on the shared server) is pre-existing/sibling-state,
  untouched by this change (footprint: datastore only).
...
```
 ...
 .pine/tickets/FEAT-gxppx1.md                       |  104 +
 ...
 .pine/tickets/FEAT-nrfg6e.md                       |  123 +
 ...
 .pine/tickets/FEAT-wkmv5e.md                       |   88 +
 ...
 internal/repository/executions.go                  |  236 +-
 internal/repository/models.go                      |  278 +-
 ...
 785 files changed, 138152 insertions(+), 2844 deletions(-)
```
