...
## Work notes (SDKDatastore, 2026-09-06)

Scope kept to `sdk/` only: `sdk/src/server.ts` (13 methods + filter/pagination
types), `sdk/src/http.ts` (`accept`/`contentType` transport options),
`sdk/test/server.test.ts`, `sdk/test/operation-coverage.test.mjs`,
`sdk/test/datastore-live.test.mjs`, `sdk/README.md`. No Go, no embed changes —
the 4 methods from FEAT-1c70nt (`getDatastore`, `listDatastoreRows`,
`getDatastoreRow`, `insertDatastoreRow`) are untouched and read-only.

Per-op proof (each maps one server operationId to one client method; the gate
asserts the method exists and the stubbed-fetch tests assert the wire shape):

- `list-datastores` → `listDatastores`: `GET /datastores`, body `{items}`.
- `create-datastore` → `createDatastore`: `POST /datastores`,
  `{name, columns?}` with closed `DatastoreColumnType`; columns omitted when
  absent (name-only create sends `{name}`).
- `rename-datastore` → `renameDatastore`: `PUT /datastores/{id}`, `{name}`.
- `delete-datastore` → `deleteDatastore`: `DELETE /datastores/{id}`, 204 → void.
- `clear-datastore` → `clearDatastore`: `POST /datastores/{id}/clear`, `{deleted}`.
- `add-datastore-column` → `addDatastoreColumn`: `POST /{id}/columns`, `{name, type}`.
- `rename-datastore-column` → `renameDatastoreColumn`: `PUT /{id}/columns/{name}`,
  `{name: newName}`. No retype exists server-side (type fixed at creation).
- `delete-datastore-column` → `deleteDatastoreColumn`: `DELETE /{id}/columns/{name}`,
  204 → void.
- `update-datastore-rows` → `updateDatastoreRows`: `PUT /{id}/rows`,
  `{filter, values}` → `{matched, rows}`.
- `delete-datastore-rows` → `deleteDatastoreRows`: `DELETE /{id}/rows`,
  `{filter}` → `{deleted, rows}`.
- `upsert-datastore-row` → `upsertDatastoreRow`: `POST /{id}/rows/upsert`,
  `{filter, values}` → `{inserted, matched, rows}`.
- `export-datastore-rows` → `exportDatastoreRows`: `GET /{id}/rows/export`,
  `Accept: text/csv`, resolves the file as text; `includeSystemColumns` opt-in.
- `import-datastore-rows` → `importDatastoreRows`: `POST /{id}/rows/import`,
  raw `text/csv` body (verbatim, never JSON-quoted) → `CSVImportReport`.

Acceptance criteria, per box:

- [x] All three resource groups reachable: 13 methods above, proven by
  `datastore management`, `datastore columns`, `filtered row writes` and
  `datastore CSV transfer` describes in `sdk/test/server.test.ts`
  (method + path + body per op, 204 → void, id/column encoding).
- [x] Closed filter: `datastoreFilter('and'|'or', conditions)` produces the
  wire `{type, filters}` verbatim (the shape `listDatastoreRows` triples
  express); `DatastoreFilterOperator` is the 10-condition union and
  `isEmpty`/`isNotEmpty` withhold the value slot. A misspelt operator fails
  to compile; the `datastore filters` describe pins the envelope.
- [x] Delete requires a filter: the signature takes it positionally, and an
  empty `filters: []` or `filters: null` rejects client-side on update,
  delete and upsert with zero fetches sent. The refusal message names only
  the operation — a sentinel value test proves row contents never land in it.
- [x] One `paginateCursor` helper drives both `iterateDatastoreRows<TRow>`
  and `iterateExecutions`; tests script two-page walks (cursor forwarded,
  exhaustion at missing `nextCursor`) for each.
- [x] Row typing is honest: `DatastoreRow = Record<string, unknown>` default
  with a `TRow` parameter for host-declared schemas; `DatastoreColumnType`
  closes create/column types; dates documented as strings, booleans as bools.
- [x] Dry-run: the server exposes no dry-run parameter (the engine is called
  with `false`; the REST envelopes carry only `matched`/`deleted`/`inserted`
  + rows), so there is nothing to expose — the result envelopes are typed
  as declared and the README states the absence plainly.
- [x] No duplication: the 4 embed methods are byte-identical (one accidental
  overwrite of `getDatastoreRow` during editing was restored and verified via
  `git diff` showing a purely additive change); the stale "belongs to a later
  ticket" pointer now names the management block below.
- [x] README documents the SQL-node guard ("A workflow SQL node can never
  read a datastore"), the embed/backend split, the filter rule, the
  iterator, the CSV pair, and the operation count 46 → 63.
- [x] Credential behavior kept: `accept`/`contentType` ride the existing
  header path, `apiKey` → `Bearer` and explicit-`Authorization`-wins are
  asserted per group (management, columns, filtered writes, transfer).

Proof, run by hand 2026-09-06 (all green):

- `pnpm --dir sdk check` clean (tsc).
- `pnpm --dir sdk test` → 6 files, 81 tests, all pass — including the
  operation-coverage gate (0 uncovered, 0 stale; previously exactly these 13
  listed as missing) and the new live CSV-export round-trip over real HTTP.
- `pnpm --dir sdk build` clean (tsc build).
- `make sdk-version-check` green (0.1.0 == 0.1.0; no bump — additive surface
  ships under the existing pre-1.0 line, release numbering stays with
  FEAT-3taswf).
...
```
 ...
 .pine/tickets/FEAT-nc6z9r.md                       |  150 +
 ...
 internal/interop/n8n/corpus/baseline.json          |  118 +-
 ...
 internal/interop/n8n/n8n_test.go                   | 2469 ++++++++++++-
 internal/interop/n8n/parameters.go                 | 3224 ++++++++++++++++-
 ...
 sdk/src/generated/models.ts                        | 2551 ++++++++++++-
 sdk/src/http.ts                                    |   55 +-
 ...
 web/src/lib/api/generated/models/index.ts          |   53 +
 ...
 851 files changed, 156713 insertions(+), 2526 deletions(-)
```
