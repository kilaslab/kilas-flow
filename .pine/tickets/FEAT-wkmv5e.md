...
Status: implemented (online-add scope per batch assignment), awaiting Main
verification. No engine DDL changes needed: AddColumn was already one
`ALTER TABLE ... ADD` statement per dialect (schema-text edit, never a
rebuild), so this ticket's work is the proof plus row-level coverage.

Tests (`evolve_test.go`, sqlite + PG16 kf-pg-vector:55434 green):
- `TestAddColumnDoesNotStallConcurrentReaders`: 8 goroutines x 25 ReturnAll
  reads concurrent with 5 sequential AddColumns — zero failures, zero
  partial row sets; old rows read new columns as null afterwards.
- `TestAddColumnReadsBackNullForExistingRows` and
  `TestRenamePreservesValuesAndOrdering` (values + catalogue position
  order survive rename).

Deliberate deferrals (batch narrowed evolution to add-column-online):
tombstoned delete, name-reuse-after-delete, retype queueing, maintenance
reclaim action + report, and per-restricted-case refusal tests stay open
for the follow-up that owns them. DropColumn remains direct DDL on both
drivers (transactional), documented as such.
...
```
 ...
 785 files changed, 139748 insertions(+), 2844 deletions(-)
```
