...
## Notes (VectorTier, 2026-09-06)

- Landed as 000006_vector_store (both dialects, up+down); version agreed with LongtailSecrets (theirs: 000008_secret_bindings; 000007 free). No new dependency: vectors bind as `[a,b,c]` text literals with `::vector` casts per the plan, so no licence note is owed (memory/licensing.md checked before adding anything — nothing added).
- Schema per plan option 3: `vector_collections` catalogue + fixed tables `vector_documents_{384,768,1024,1536}`; per table one HNSW + one IVFFlat index for each of cosine/L2/IP (24 ANN indexes) + a `(tenant_id, collection_id)` btree. Longest identifier 37B, inside the 63B budget under a 16B prefix. SQLite half carries only the catalogue (no vector type there); both nodes refuse on it via VectorUnavailableReason.
- Re-runnable per Main: all CREATEs IF NOT EXISTS, all DROPs IF EXISTS (both dialects). The 23505 concurrent-create race stays closed in the migration runner (re-reads applied versions after failure), noted in the up-file header. Repository-suite 42P07 on rerun is fixed by this.
- Ownership note: only new files touched (nodes/pgvector*.go, new migrations) + one additive GLYPHS line. Registration NOT wired (RegisterAll/RegisterExecutors are outside my ownership): Main must add `EmbeddingsNode(reason)` / `VectorStoreNode(reason)` to RegisterAll and `NewEmbeddingsExecutor(policy, store)` / `NewVectorStoreExecutor(store)` to RegisterExecutors, with `reason = VectorUnavailableReason(cfg.Database.Driver)` refined by `ProbeVectorExtension` on postgres. (An earlier range-PUT of mine clobbered 6 GLYPHS lines; Main restored them — additive-only on node-visual.ts from here.)
- PG-gated tests use `-p 1` for combined runs (shared-server race per memory/persistence.md); each test migrates a throwaway `kf_vector_test` DB twice (rerun proof baked in) under prefix `kvtest_`, then drops it.
- Evidence: `go test ./nodes/...` green; PG suite green on own pgvector/pgvector:pg17 (127.0.0.1:55436) and shared kf-pg-vector (127.0.0.1:55434); PG tests skip cleanly without DSN and without the extension. ANN proof: 5000-row cosine collection, plan shows `Index Scan using kvtest_idx_vdocs384_ivf_cos` (planner's pick among the cosine ANN indexes; HNSW+IVFFlat both exist per distance). Query plan captured via `ExplainSearch` (SET LOCAL enable_seqscan=off, same statement Search runs).
- Exposed for wiring/tests: `VectorStore`/`PostgresVectorStore`/`DisabledVectorStore`, `VectorUnavailableReason`, `ProbeVectorExtension`, `ExplainSearch`.
...
```
 ...
 .pine/tickets/FEAT-k65hqv.md                       |    70 +
