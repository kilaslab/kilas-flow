...
- 2026-09-24: Any change to internal/jsrun (or the importer's Code-node mapping) that moves a Code-node corpus outcome must regenerate the corpus baseline in the SAME change: make js-corpus && make js-corpus-baseline, committing internal/jsrun/corpus/BASELINE.md and baseline.json, and the commit says which bodies moved and why. CI holds every verified body to baseline.json and fails on any move, better or worse (FEAT-afkx3k).
- 2026-09-24: internal/jsrun/corpus/jsdiff_test.go (make js-diff) imports os/exec and reads and writes files, and corpus_test.go/scoreboard_test.go read files, under internal/jsrun/. That is test-only and deliberate: the package's only non-test file is doc.go with no imports, and the runtime guardrail (internal/guardrails/jsruntime_boundary_test.go) skips _test.go files. If that guardrail is ever tightened to cover test files, exempt internal/jsrun/corpus/ rather than moving the instrument out (FEAT-afkx3k).
