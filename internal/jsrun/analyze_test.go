...
func TestRequireOfAModuleNotShippedIsRefused(t *testing.T) {
	...
	analysis := accepted(t, "const crypto = require('node:crypto')\nconst _ = require('lodash')\nconst { DateTime } = require('luxon')\nreturn []")
	if strings.Join(analysis.Requires, ",") != "crypto,lodash,luxon" || !analysis.UsesLuxon {
		t.Fatalf("analysis = %#v, want the three shipped modules and Luxon", analysis)
	}
