...
// parseZoneNamesOverlay parses a per-locale zone-name overlay table (the
// same "@long standard|long daylight|short standard|short daylight" shape
// as zoneTableText, generated the same way) into canonical zone id, in
// lower case, to names. An overlay holds only the zones whose long or
// short name differs from zoneTable's (en-US-based) one for that locale;
// zoneName falls back to zoneTable's own names for every zone missing here.
func parseZoneNamesOverlay(text string) map[string]zoneNames {
	table := map[string]zoneNames{}
	var names zoneNames
	for _, line := range strings.Split(text, "\n") {
		...
		for _, entry := range strings.Fields(line) {
			...
			table[strings.ToLower(canonical)] = names
		}
	}
	return table
}

// zoneNamesCA and zoneNamesGB are en-CA's and en-GB's zone-name overlays.
// Only 'short' and 'long' vary by locale — 'shortOffset'/'longOffset' are
// computed straight from the UTC offset for every locale, see offsetName.
var zoneNamesCA = sync.OnceValue(func() map[string]zoneNames { return parseZoneNamesOverlay(zoneTableTextCA) })
var zoneNamesGB = sync.OnceValue(func() map[string]zoneNames { return parseZoneNamesOverlay(zoneTableTextGB) })
...
// localeZoneNames is zone's long/short names as locale prints them: its
// overlay's names where it has an entry for zone.canonical (differing from
// en-US's — see zoneNamesCA/zoneNamesGB), zone's own (en-US-based) names
// otherwise, for en-US and for every zone neither overlay lists.
func localeZoneNames(locale *dateLocale, zone *resolvedZone) zoneNames {
	var overlay map[string]zoneNames
	switch locale.tag {
	case "en-CA":
		overlay = zoneNamesCA()
	case "en-GB":
	...
	if names, ok := overlay[strings.ToLower(zone.canonical)]; ok {
		return names
	}
	return zone.names
}
...
// BEGIN zone table (en-CA overlay) (generated by scripts/js-parity/record.mjs; do not edit)
const zoneTableTextCA = `
@French Southern and Antarctic Time|||
Indian/Kerguelen
@Newfoundland Standard Time|Newfoundland Daylight Time|NST|NDT
America/St_Johns Canada/Newfoundland=America/St_Johns
@Saint-Pierre-et-Miquelon Standard Time|Saint-Pierre-et-Miquelon Daylight Time|PMST|PMDT
America/Miquelon
@Wallis and Futuna Time|||
Pacific/Wallis
`

// END zone table (en-CA overlay)

// BEGIN zone table (en-GB overlay) (generated by scripts/js-parity/record.mjs; do not edit)
const zoneTableTextGB = `
...
America/St_Lucia America/St_Thomas America/St_Vincent America/Thule America/Tortola
America/Virgin=America/St_Thomas Atlantic/Bermuda Canada/Atlantic=America/Halifax
@Central European Standard Time|Central European Summer Time|CET|CEST
Africa/Algiers Africa/Ceuta Africa/Tunis Arctic/Longyearbyen Atlantic/Jan_Mayen=Arctic/Longyearbyen
CET=Europe/Brussels Europe/Amsterdam Europe/Andorra Europe/Belgrade Europe/Berlin Europe/Bratislava
Europe/Brussels Europe/Budapest Europe/Busingen Europe/Copenhagen Europe/Gibraltar Europe/Ljubljana
Europe/Luxembourg Europe/Madrid Europe/Malta Europe/Monaco Europe/Oslo Europe/Paris Europe/Podgorica
...
Canada/Central=America/Winnipeg Canada/Saskatchewan=America/Regina Mexico/General=America/Mexico_City
US/Central=America/Chicago US/Indiana-Starke=America/Indiana/Knox
@Eastern European Standard Time|Eastern European Summer Time|EET|EEST
Africa/Cairo Africa/Tripoli Asia/Beirut Asia/Famagusta Asia/Gaza Asia/Hebron Asia/Nicosia EET=Europe/Athens
Egypt=Africa/Cairo Europe/Athens Europe/Bucharest Europe/Chisinau Europe/Helsinki Europe/Kaliningrad
Europe/Kiev Europe/Kyiv=Europe/Kiev Europe/Mariehamn Europe/Nicosia=Asia/Nicosia Europe/Riga Europe/Sofia
Europe/Tallinn Europe/Tiraspol=Europe/Chisinau Europe/Uzhgorod=Europe/Kiev Europe/Vilnius
...
America/Adak America/Atka=America/Adak HST=Pacific/Honolulu Pacific/Honolulu Pacific/Johnston=Pacific/Honolulu
...
@St Pierre & Miquelon Standard Time|St Pierre & Miquelon Daylight Time||
America/Miquelon
@Western European Standard Time|Western European Summer Time|WET|WEST
Atlantic/Canary Atlantic/Faeroe Atlantic/Faroe=Atlantic/Faeroe Atlantic/Madeira Europe/Lisbon
Portugal=Europe/Lisbon WET=Europe/Lisbon
`

// END zone table (en-GB overlay)
...
// enCA and enGB (below) are en-CA's and en-GB's date data: Canadian and
// British English, CLDR as Node 24 (ICU 78) renders it, derived from Node's
// actual Intl output rather than transcribed from CLDR source or training
// data — the same principle the rest of this package follows (see doc.go),
// applied to data no clean-room script fully automates. What follows is the
// procedure that produced the two tables below, precise enough to redo
// after an ICU bump changes what Node prints; unlike the zone-name tables
// and the pattern generator, it was not feasible to encode this fully into
// record.mjs (see FEAT-9we7kw's ticket notes for why: the two rules below —
// quirky and lockedIn — took several rounds of hypothesise-and-verify
...
//     labels.
//  2. For each multi-field key, ask Node for the field order and literal
//     separators the target locale actually uses: build the
//     Intl.DateTimeFormat options that skeleton represents (letter runs
//     translate to option values the same way optionsSkeleton's widths map
//     does, in reverse) and read formatToParts(). Single-field keys
//     ("G", "M", "MMM", "E", "d", "a", "m", "s", "S") cannot reorder or
//     gain a separator and are copied unchanged.
//  3. For the width of each field, do not trust the requested option back
//     — ask resolvedOptions() (or read formatToParts' value padding
//     directly). Node does not always render what was asked: en-US's own
//     bare "H" (hour: 'numeric') already resolves to hour: '2-digit', and
//     en-CA's bare "yMd" (year/month/day: 'numeric') resolves to
//     year: 'numeric', month: '2-digit', day: '2-digit'. adjust() below
//     calls this a "quirky" field: one whose matched token width differs
//     from what its skeleton key declares.
//  4. A quirky field's override does not always carry into a request that
//     does not exactly match its key — verify each width combination a
//     quirky field could plausibly appear in (2-digit vs 'numeric', with
//     and without a weekday, an era, a zone, a different hour cycle)
//     directly against Node, not by extrapolating one rule. Two rules
//     were found this way, both implemented in adjust():
//     - Without a weekday, or an era joined to a year, in the same
//     pattern: the override carries only when *every* quirky field the
//     request asks for matches its own key's width too (en-CA's bare
//     "Md": a request mixing 'numeric' and '2-digit' for month and day
//     gets neither override — "3/01", never "03/01" or "03/1").
//     - With a weekday, or an era joined to a year: the override always
//     carries, regardless of what the rest of the request asks
//     (en-CA's "MEd" keeps zero-padded month and day even when one is
//     requested 'numeric'; en-GB's "GyMd" the same, unlike quirk-free
//     "GyM").
//  5. Some locales need anchors beyond the shared 47 keys, because Node
//     genuinely stores a different pattern for one specific width
//     combination that no amount of adjusting the 47 keys' patterns can
//     reach — en-GB's "Hmm"/"Hmmss"/"k"-family entries below are this
//     case, each found by noticing a combination the 47-key table
...
//     generates (1,794 of them, in two zones), and
//     TestDatesMatchTheRecordedNodeGoldens covers dateStyle/timeStyle,
//     locale matching and the few probes in record.mjs's dateProbes.
//     A change here is only correct once both are green with `node
//     scripts/js-parity/record.mjs --check` reporting no drift.
...
var enGB = sync.OnceValue(func() *dateLocale {
	...
	locale.available = newDateLocale("en-GB", [][2]string{
		...
		// changes the rule again: "Hmv" (no seconds) still pads, and once
		// seconds join it, "timeZoneName: 'short'/'long'/'shortOffset'"
		// unpad the hour regardless of the seconds' own width, but
		// 'longOffset' only unpads it when the seconds are also 2-digit
		// ("03:45:00 GMT+00:00" for 'numeric' seconds, "3:45:00
		// GMT+00:00" for '2-digit' ones) — verified directly against
		// Node. A dedicated anchor for that one width combination was
		// tried and reverted: the matcher's width metric puts an
		// "OOOO"-keyed (longOffset) entry numerically closer to a bare
		// "z"/"O" (short/shortOffset) request than "Hmsv"'s "v" is,
		// so it silently won those requests too and padded them wrongly.
		// nativeDateTimeFormat refuses the narrow shape by name instead
		// (hour + minute + 'numeric' — not '2-digit' — seconds +
		...
		// minute width), and hour+minute+second pads only when both
		// minute and second are 'numeric' ("09:07:03" — "kms" below);
		// either at '2-digit' unpads it ("9:07:03" — "kmss"/"kmms").
		// Present with no minute at all (an hour+second request, or an
		// hour request with fractionalSecondDigits and no minute) Node
		// breaks the tie between "primary: hour, append: second" and
		// "primary: second, append: hour" the opposite way every other
		// hour cycle does; that tie-break is not a per-field width rule
		// and is refused by name in nativeDateTimeFormat instead of
		// guessed at here.
		{"k", "kk"}, {"km", "k:mm"}, {"kms", "kk:mm:ss"}, {"kmss", "k:mm:ss"}, {"kmms", "k:mm:ss"},
	}).available
...
func resolveDateLocale(requested []string) (tag language.Tag, name string, err error) {
	...
	scriptName, regionName := script.String(), region.String()
	if base.String() != "en" || (scriptName != "Zzzz" && scriptName != "Latn") ||
	...
	// An explicit script subtag ("en-Latn-GB") is not "en-GB": the BCP 47
	// Lookup algorithm ECMA-402 uses strips the tag from the right, one
	// subtag at a time, so "en-Latn-GB" tries "en-Latn-GB", "en-Latn",
	// then "en" — never "en-GB", which only appears on the fallback path
	// when no script was given in the first place. Node resolves
	// en-Latn-GB, en-Latn-CA and en-Latn-US alike to plain "en".
	name = "en"
	if scriptName == "Zzzz" {
		switch regionName {
		case "US":
...
// family24 reports whether an hour cycle is one of the two 24-hour-family
// cycles (h23, h24) rather than the 12-hour family (h11, h12).
func family24(cycle string) bool { return cycle == "h23" || cycle == "h24" }

// nativeHourCycle resolves the hour cycle exactly as ECMA-402's real
// InitializeDateTimeFormat does (verified directly against Node — fix
// round 2 implemented a simpler "hour12 always forces h12/h23 outright"
// shortcut, which fix round 3's review found wrong whenever hour12 is
// combined with a -u-hc- extension naming the OTHER member of the same
// family): an explicit hourCycle option is first discarded if hour12 was
// also given (hour12 always takes priority when both are present); what
// remains (the surviving hourCycle option, else the extension, else the
// locale's own default) is "hcDefault" — and hour12, if given, does not
// unconditionally pick h12/h23; it forces the 12-/24-hour FAMILY, but
// keeps hcDefault's own specific variant (h11 instead of h12, h24 instead
// of h23) when hcDefault already belongs to that family. E.g. en-GB-u-hc-h11
// with hour12: true resolves to h11 (Node: midnight "0:05 am"), not h12
// (which this runtime gave at head, midnight "12:05 am") — hcDefault
// (from the extension) is already in the 12-hour family hour12 asks for,
// so it is kept outright.
func nativeHourCycle(locale *dateLocale, options dateTimeOptions, extensionCycle string) string {
	requested := options.hourCycle
	if options.hour12 != nil {
		requested = ""
	}
	hcDefault := requested
	if hcDefault == "" {
		hcDefault = extensionCycle
	}
	if hcDefault == "" {
		hcDefault = locale.defaultHourCycle
	}
	if options.hour12 == nil {
		return hcDefault
	}
...
func nativeDateTimeFormat(args []any) (any, error) {
	...
	// The hour cycle: see nativeHourCycle's doc comment above for the
	// algorithm and why fix round 2's simpler shortcut was wrong. Whether
	// the resolved locale name carries "-u-hc-<extensionCycle>" is decided
	// below, once it is known whether hour is even part of the resolved
	// output at all — a bare dateStyle (no hour anywhere) drops the
	// extension entirely (verified directly against Node: "en-GB", never
	// "en-GB-u-hc-h11", for a dateStyle-only request even with an explicit
	// hourCycle option or an en-GB-u-hc-h11 tag) — and otherwise only when
	// the negotiated cycle actually equals what the tag's own extension
	// asked for (also verified directly: the resolved locale drops the
	// extension the moment anything overrode it away from its own
	// request, whether by family-override or by an outright different
	// explicit hourCycle option).
	extensionCycle := unicodeExtension(tag, "hc")
	cycle := nativeHourCycle(locale, options, extensionCycle)
	...
	if options.dateStyle != "" || options.timeStyle != "" {
		...
		// never wrong. Fix round 2's version of this refusal (any
		// effective second present, any minute width, shortOffset only at
		// 'numeric' second width) over-refused 377 of 584 cases fix round
		// 3's review found actually already match Node — disabling the
		// refusal and diffing the *entire* sweep against Node directly
		// (not guessed) gives the real, narrower shape, which turns out
		// to depend on the hour cycle too (never h11/h12; h23 and h24
		// differ from each other):
		//   - 'shortOffset' breaks only at hourCycle h23 (the default, or
		//     explicit), only with 'numeric' (not '2-digit') minute, and
		//     only when the effective second (explicit, or implied by
		//     fractionalSecondDigits — see forMatching) is not '2-digit'.
		//   - 'longOffset' at hourCycle h23 breaks whenever the effective
		//     second is present, at 'numeric' OR '2-digit' minute —
		//     except 'longOffset' with a '2-digit' minute AND a
		//     '2-digit' effective second together, which does not break.
		//   - 'longOffset' at hourCycle h24 breaks only with 'numeric'
		//     minute and a non-'2-digit' effective second — narrower than
		//     h23: a '2-digit' minute, or a '2-digit' effective second,
		//     each independently keep it correct.
		// No anchor can express any of these widths without also, and
		// wrongly, padding a case that already matches Node (tried and
		// reverted for 'longOffset' alone in fix round 1) — refused by
		// name instead, precisely scoped to what fix round 3's full,
		...
		if !liftedDateRefusals.Load() && locale.tag == "en-GB" && requestedSkeleton.has(fieldHour) && requestedSkeleton.size[fieldHour] == 1 &&
			...
			if effectiveSecond {
				...
				h24 := hourChar == 'k'
				minuteWidth1 := requestedSkeleton.size[fieldMinute] == 1
				var breaks bool
				switch {
				case !longOffset:
					breaks = !h24 && minuteWidth1 && effectiveSecondWidth != 2
				case h24:
					breaks = minuteWidth1 && effectiveSecondWidth != 2
				default:
					breaks = minuteWidth1 || effectiveSecondWidth != 2
				}
				if breaks {
	...
	// A tag extension with no hourCycle/hour12 option at all to possibly
	// override it stays in the resolved locale name regardless of
	// whether hour even appears in the output (a dateStyle-only request,
	// say) — Node reports "en-GB-u-hc-h11" for that, verified directly.
	// Once an explicit option is given, the extension shows only when it
	// is what's actually in effect (cycle equals it) AND hour is part of
	// the output; a dateStyle-only request with an explicit hourCycle
	// option — even one equal to the extension's own value — drops the
	// extension (Node: plain "en-GB"), since dateStyle-only never
	// consults the hour cycle for anything observable.
	hasHourCycleOption := options.hourCycle != "" || options.hour12 != nil
	if extensionCycle != "" && (!hasHourCycleOption || (shown.has(fieldHour) && cycle == extensionCycle)) {
...
// generator rebuild (numeric width 1, or 2-digit width 2) — a decision
// genuinely independent of the width rule optionsSkeleton/bestPattern
// already gets right for a plain component request (hour: 'numeric' etc.),
// verified directly against Node across every combination of a locale's
// own -u-hc- extension, an explicit hourCycle option and hour12, at every
// resulting cycle. Two rules, one per locale family, both confirmed with
// zero exceptions across that full cross-product:
//   - A locale whose own DEFAULT cycle is 24-hour (en-GB): the tag's own
//     -u-hc- extension alone decides, ignoring any hourCycle/hour12
//     option entirely — width 1 iff the extension names a 12-hour-family
//     cycle (h11 or h12); width 2 otherwise (no extension, or the
//     extension itself names h23/h24). E.g. en-GB-u-hc-h11 stays width 1
//     even once an explicit hourCycle: 'h24' option overrides the actual
...
// en-CA, h23 for en-GB); an hour cycle other than that default, or a width
// stylePatternPadsHour says disagrees with the default pattern's own width,
// rebuilds the time through the generator instead, as V8 does, so en-US
// h23 gives "HH:mm" and drops the AM/PM, and en-GB h12 gives "hh:mm a" and
// gains it.
func (locale *dateLocale) stylePattern(dateStyle, timeStyle, cycle, extensionCycle string) []patternToken {
	...
	if timeStyle != "" {
		...
		targetWidth := 1
		if stylePatternPadsHour(locale, extensionCycle, cycle) {
			targetWidth = 2
		}
		s := skeletonOfTokens(clock)
		if cycle != locale.defaultHourCycle || s.size[fieldHour] != targetWidth {
			s.char[fieldHour] = map[string]byte{"h11": 'K', "h12": 'h', "h23": 'H', "h24": 'k'}[cycle]
			s.size[fieldHour] = targetWidth
			s.char[fieldDayPeriod], s.size[fieldDayPeriod] = 0, 0
...
func (locale *dateLocale) render(tokens []patternToken, at time.Time, zone *resolvedZone) ([]any, error) {
	...
	for _, token := range tokens {
		...
		case 'z', 'O', 'v':
			...
			name, err := zoneName(locale, at, zone, style)
			if err != nil {
				return nil, err
			}
