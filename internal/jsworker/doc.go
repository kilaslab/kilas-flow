...
//     kill it first when memory runs out (oom_score_adj 1000), and dies with
//     the server (a parent-death signal);
//   - a worker's environment holds only the marker that makes it one and
//     the time zone settings, none of the server's configuration or secrets,
//     and it exits when its stdin closes.
//
