...
- Scope where it may be sent. `allowedDomains` holds the hosts a credential may reach; an empty list is unrestricted, an exact entry matches one host, and a `*.` entry matches subdomains but never the bare parent (internal/credentials/credentials.go, `AllowsHost`; docs/src/content/docs/concepts/credentials.md). This narrows the instance egress policy, never replaces it.
