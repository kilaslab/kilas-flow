...
- `kilasflow.datastoreTool` is the read-only agent tool: it binds one table, freezes that table's columns into a closed schema, answers filtered reads from the fixed ten-operator vocabulary, and caps a call at 50 rows by default, 200 rows and 256 KiB (nodes/datastore.go).
