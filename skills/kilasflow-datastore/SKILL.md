...
- On a write tool the author's operator is final and the model fills only the value it compares, so `like`, `ilike`, `gt`, `gte`, `lt` or `lte` against a model-filled `keyValue` can still match many rows (a `%` under `like` matches them all): use `eq` on a column that names one row when a write is meant to touch one (nodes/datastore.go).
