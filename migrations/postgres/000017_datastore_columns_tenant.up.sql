...
-- insert that forgets the tenant; the Go layer does (the engine sets it on every
-- column it writes and filters every read on it), and the tenant purge test
-- asserts the column.

ALTER TABLE "datastore_columns" ADD COLUMN "tenant_id" varchar(64) NOT NULL DEFAULT '';
...
CREATE INDEX IF NOT EXISTS "idx_datastore_columns_tenant" ON "datastore_columns" ("tenant_id","datastore_id");
