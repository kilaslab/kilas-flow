...
--
-- bytea rather than jsonb, matching executions.input/output/error: the report
-- is written once by the adapter that produced it and read back whole, never
-- queried into, so PostgreSQL would be parsing and validating JSON on every
-- write for no reader's benefit.
--
...
ALTER TABLE "workflow_versions" ADD COLUMN "diagnostics" bytea;
