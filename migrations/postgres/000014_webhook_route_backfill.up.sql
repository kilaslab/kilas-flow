...
-- changes nothing.
--
-- The route is 32 lowercase hex characters, the format the application mints, so
-- a backfilled route cannot be told apart from any other. It is a UUIDv4 with its
-- dashes removed: gen_random_uuid() is in core from PostgreSQL 13 and needs no
-- extension. That carries 122 random bits rather than the 128 the application
-- draws, because a UUIDv4 fixes six bits for its version and variant; the route
-- is still far beyond guessable.
--
...
SELECT "b"."tenant_id", "b"."workflow_id", "b"."node_id", replace(gen_random_uuid()::text, '-', ''), now()
