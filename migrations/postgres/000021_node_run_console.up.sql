...
-- and time, and whether output past the node's console limit was dropped.
--
-- bytea rather than jsonb, matching execution_node_runs.input/output/error and
-- response: the value is written whole by the row write that persists the
-- node's input and output and read back whole by the execution detail page,
-- never queried into, so jsonb would only add parse and validate work on every
-- write.
--
...
ALTER TABLE "execution_node_runs" ADD COLUMN "console" bytea;
