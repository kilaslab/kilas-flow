...
--
-- actor_meta is bytea/blob like this schema's other JSON payload columns
-- (workflow_versions.definition and diagnostics, credentials.public_fields):
-- written once by the caller that produced it and read back whole, never
-- queried into, so a structured column would only add parse work to every
-- write.
--
...
ALTER TABLE `workflow_versions` ADD COLUMN `actor_kind` text;

ALTER TABLE `workflow_versions` ADD COLUMN `actor_label` text;

ALTER TABLE `workflow_versions` ADD COLUMN `actor_key_id` text;

ALTER TABLE `workflow_versions` ADD COLUMN `actor_meta` blob;

UPDATE `workflow_versions`
   SET `actor_kind` = 'user', `actor_label` = `created_by`
 WHERE `created_by` IS NOT NULL AND `created_by` <> '';

ALTER TABLE `workflow_publish_events` ADD COLUMN `actor_kind` text NOT NULL DEFAULT "";

ALTER TABLE `workflow_publish_events` ADD COLUMN `actor_label` text NOT NULL DEFAULT "";

ALTER TABLE `workflow_publish_events` ADD COLUMN `actor_key_id` text NOT NULL DEFAULT "";
