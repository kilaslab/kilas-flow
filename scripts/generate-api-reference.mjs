...
const groups = [
	{ slug: 'workflows', title: 'Workflows', blurb: 'Create, read, update, run, activate, and version workflows.', operations: ['create-workflow', 'list-workflows', 'get-workflow', 'update-workflow', 'delete-workflow', 'run-workflow', 'activate-workflow', 'deactivate-workflow', 'list-workflow-versions', 'get-workflow-version', 'publish-workflow-version', 'restore-workflow-version', 'list-workflow-publish-events', 'workflow-diagnostics'] },
...
function renderErrors(version, schemas) {
	...
	lines.push(banner(version));
	lines.push('');
	lines.push('Every non-success response is an RFC 9457 problem document (`application/problem+json`). The shape is stable; the human-readable strings inside it are not — never match on `detail` text. Every operation declares its failures as the `default` response, so the table below applies to the whole API.');
	lines.push('');
