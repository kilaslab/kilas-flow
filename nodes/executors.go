...
func RegisterExecutors(registry *engine.Registry, httpPolicy safehttp.Policy, databaseGuard sqlnode.Guard, agentRuntime ai.AgentRuntime, agentMemory ai.Memory, codeCompiler runcode.Compiler, options ...ExecutorOption) error {
	...
	for id, executor := range map[string]engine.Executor{
		...
		AgentExecutorID:                  NewAgentExecutor(agentRuntime, httpPolicy, agentMemory, WithModelTimeoutCeiling(settings.modelTimeoutCeiling), WithDatastoreStore(datastoreStoreOf(settings.datastoreEngine)), WithAgentVectorStore(settings.vectorStore, NewEmbeddingsExecutor(httpPolicy, settings.vectorStore)), WithAgentSQLGuard(databaseGuard)),
		...
		VectorStoreExecutorID:            NewVectorStoreExecutor(settings.vectorStore).WithEmbedder(NewEmbeddingsExecutor(httpPolicy, settings.vectorStore)),
		DocumentLoaderExecutorID:         engine.ExecutorFunc(executeDocumentLoader),
		TextSplitterExecutorID:           engine.ExecutorFunc(executeTextSplitter),
		ExtractFromFileExecutorID:        engine.ExecutorFunc(executeExtractFromFile),
		VectorStorePGVectorExecutorID:    NewVectorStorePGVectorExecutor(NewEmbeddingsExecutor(httpPolicy, settings.vectorStore), databaseGuard),
		GoogleDriveExecutorID:            NewGoogleDriveExecutor(httpPolicy),
		GoogleDriveTriggerExecutorID:     GoogleDriveTriggerExecutor{},
		GmailExecutorID:                  NewGmailExecutor(httpPolicy),
		GmailTriggerExecutorID:           GmailTriggerExecutor{},
